...
/* $Id: ansys_component.c,v 1.6 2003/07/22 09:09:52 nagatani Exp $ */
...
static RC input_element_node( int *node, int num, FEM_ELEMENT *elem );
static void ansys_num_bc( int *numnod, int labnod, int num_bc, FEM_BC *bc );
...
RC ansys_input_node( FILE *fp, FEM_NODE_ARRAY *node )
{
	...
	RC_TRY(allocate_fem_node_array(0, node));
...
static RC input_element_node( int *node, int num, FEM_ELEMENT *elem )
{
	...
	if( (type_num == 42) || (type_num == 141) ) {
		if ( node[2] == node[3] ) {
			...
			elem[num].node_num = 3;
			for( ii = 0 ; ii < elem[num].node_num ; ii++ ) {
				elem[num].node[ii] = node[ii];
			}
		} else {
...
RC ansys_output_element( FILE *fp, FEM_ELEMENT_ARRAY elem )
{
	...
	for( ii1 = 0 ; ii1 < elem.size ; ii1++ ) {
		...
		if(type == ELEM_TRI1){
			fprintf( fp,"EN,R5.5,ATTR,      4,%7d,%7d,%7d,%7d,      0,%7d,      0, 0, 0\n",
			elem.array[ii1].material,elem.array[ii1].i_info[0],
			elem.array[ii1].physical,elem.array[ii1].label,elem.array[ii1].label); 
		...
		if(type == ELEM_TRI2){
			fprintf( fp,"EN,R5.5,ATTR,      8,%7d,%7d,%7d,%7d,      0,%7d,      0, 0, 0\n",
			elem.array[ii1].material,elem.array[ii1].i_info[0],
			elem.array[ii1].physical,elem.array[ii1].label,elem.array[ii1].label); 

			fprintf( fp,"EN,R5.5,NODE," );
			fprintf( fp,"%7d,%7d,%7d,%7d,%7d,%7d,%7d,%7d,", elem.array[ii1].node[0],
		...
		if((type == ELEM_QUAD1)||(type == ELEM_QUAD2)||(type == ELEM_HEXA1)){
			fprintf( fp,"EN,R5.5,ATTR,%7d,%7d,%7d,%7d,%7d,      0,%7d,      0, 0, 0\n",
			elem.array[ii1].node_num,elem.array[ii1].material,elem.array[ii1].i_info[0],
			elem.array[ii1].physical,elem.array[ii1].label,elem.array[ii1].label); 
			fprintf( fp,"EN,R5.5,NODE," );
			for( ii2 = 0 ; ii2 < node ; ii2++ ){
				fprintf( fp,"%7d,",elem.array[ii1].node[ii2] );
		...
		if(type == ELEM_TETRA2){
			fprintf( fp,"EN,R5.5,ATTR,     20,%7d,%7d,%7d,%7d,      0,%7d,      0, 0, 0\n",
			elem.array[ii1].material,elem.array[ii1].i_info[0],
			elem.array[ii1].physical,elem.array[ii1].label,elem.array[ii1].label); 
...
RC ansys_input_material( FILE *fp, FEM_MATERIAL_PROP_ARRAY *material )
{
	...
	RC_TRY( allocate_fem_material_prop_array(0, material) );
	...
	while(1) {
		if(((fgetidc(fp,"ssisiid",tmp)) == NORMAL_RC) && 
			...
			if( flag == 0 ) {
				init_fem_material_prop( &(material->array[counter]) );
				material->array[counter].label = tmp[4].i;
				flag++;
			} else if( flag != 0 && material->array[counter].label != tmp[4].i ) {
				counter++;
				init_fem_material_prop(&(material->array[counter]));
				material->array[counter].label = tmp[4].i;
			}
...
RC ansys_output_cdb_file( FILE *fp,
                          ...
                          FEM_BC_ARRAY force)
{
	fprintf( fp, "/COM,ANSYS RELEASE  5.6 \n" );
	fprintf( fp, "/PREP7 \n" );
	fprintf( fp, "/NOPR \n" );
	fprintf( fp, "/TITLE, \n" );
	fprintf( fp, "*IF,_CDRDOFF,EQ,1,THEN     !if solid model was read in \n" );
	fprintf( fp, "_CDRDOFF=             !reset flag, numoffs already performed \n" );
	fprintf( fp, "*ELSE              !offset database for the following FE model \n" );
	...
	fprintf( fp, "*ENDIF \n" );
	fprintf( fp, "ET, %5d,%3d \n",element.array[0].i_info[0], element.array[0].i_info[1]);
...
RC ansys_read_vp_result(FILE *fp_velo, FILE *fp_press, FEM_VELO_ARRAY *vp)
{
	...
	RC_TRY(allocate_fem_velo_array(0, vp));

	/* 流速結果ファイルの読み込み */
	while(1){
		if( fgets( buffer, sizeof(buffer) ,fp_velo ) == NULL )
			break;
		if( sscanf(buffer,"%d %le %le %le %le", &vn, &vx, &vy, &vz, &vsum) == 5 ) {
			RC_TRY( realloc_fem_velo_array(vp));
