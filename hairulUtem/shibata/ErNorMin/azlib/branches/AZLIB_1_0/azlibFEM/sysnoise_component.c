...
/* $Id: sysnoise_component.c,v 1.8 2003/07/23 04:57:45 sasaoka Exp $ */
...
static RC sysnoise_output_node( FILE *fp, FEM_NODE_ARRAY node );
static RC elem_type( int type_num, FEM_ELEM_TYPE *type );
static RC set_line( FEM_ELEMENT *element, int *node_num );
static RC set_tri( FEM_ELEMENT *element, int *node_num );
static RC set_quad( FEM_ELEMENT *element, int *node_num );
static RC set_tetra( FEM_ELEMENT *element, int *node_num );
static RC set_penta( FEM_ELEMENT *element, int *node_num );
static RC set_hexa( FEM_ELEMENT *element, int *node_num );
static RC sysnoise_output_element( FILE *fp, FEM_ELEMENT_ARRAY element );
static RC put_line( FILE *fp, FEM_ELEMENT element );
static RC put_tri( FILE *fp, FEM_ELEMENT element );
static RC put_quad( FILE *fp, FEM_ELEMENT element );
static RC put_tetra( FILE *fp, FEM_ELEMENT element );
static RC put_penta( FILE *fp, FEM_ELEMENT element );
static RC put_hexa( FILE *fp, FEM_ELEMENT element );
static int elem_type_number( FEM_ELEM_TYPE type );
...
RC sysnoise_input_pressure( FILE *fp, FEM_SOUND_PRESSURE_ARRAY *sound )
{
	...
	RC_TRY( allocate_fem_sound_pressure_array(0, sound) );
	...
	while(1){
		...
		RC_TRY( realloc_fem_sound_pressure_array(sound) );
	...
	RC_TRY( clean_fem_sound_pressure_array(sound) );
...
RC sysnoise_input_node( FILE *fp, FEM_NODE_ARRAY *node )
{
	...
	RC_TRY( allocate_fem_node_array(node_num, node) );
	node->size = node_num;
...
RC sysnoise_input_element( FILE *fp, FEM_ELEMENT_ARRAY *element )
{
	...
	RC_TRY( allocate_fem_element_array(elem_num, element) );
                                 ...
                                 FEM_SOUND_PRESSURE_ARRAY sound, int dim )
{
	...
	for(ii1=0; ii1<node.size; ii1++){
		...
		RC_NEG_CHK( index = search_fem_sound_pressure_node(sound,
