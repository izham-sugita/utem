...
/* $Id: extract_surface.c,v 1.8 2003/07/22 09:18:00 nagatani Exp $ */
...
#define FEM_MAX_NODE_LIGHT  8
#define FEM_EDGE_MAX_FROM  16
#define FEM_CUT_MAX_NODE   81
static int i_random[I_RANDOM];

typedef struct {
	FEM_ELEM_TYPE type;
	int node_num;
	int node[FEM_MAX_NODE_LIGHT];
	int from_num;
...
typedef struct {
	FEM_ELEM_TYPE type; /* ELEM_LINE1 or ELEM_LINE2 */
	int node_num;       /*     2      or     3      */
	int node[3];
	int from_num;
	int from[FEM_EDGE_MAX_FROM];   /* 元の要素label */
	int from_e[FEM_EDGE_MAX_FROM]; /* cut_element 専用パラメータ */
} FEM_ELEMENT_EDGE;
...
typedef struct {
	...
	int physical;
	int node_num;
	int node[FEM_CUT_MAX_NODE];
} FEM_ELEMENT_NODE_BOX;


/* local function */
static void init_i_random(int hash_size);
static int elem_hash_index(int hash_size, FEM_ELEMENT_FACE *hash,
...
static int cal_hash_size(FEM_ELEMENT_ARRAY elem);
static RC extract_face(FEM_ELEMENT_ARRAY elem, FEM_ELEMENT_ARRAY *face, int n);
static RC extract_face_light(FEM_ELEMENT_ARRAY elem, FEM_ELEMENT_FACE **hash,
                             int *hash_size);
static RC input_hash(FEM_ELEMENT_ARRAY elem, FEM_ELEMENT_FACE *hash,
                                             int hash_size);
static RC hash2elem(int hash_size, const FEM_ELEMENT_FACE *hash,
                    int n, FEM_ELEMENT_ARRAY *face);
static RC clean_hash(FEM_ELEMENT_FACE *hash, int *hash_size);
static RC set_hash_face(int hash_size, FEM_ELEMENT_FACE *hash,
                        int key_size, const int *keys, int label,
                        FEM_ELEM_TYPE type, int number);
static RC set_hash_tri1(int hash_size, FEM_ELEMENT_FACE *hash,
                                               FEM_ELEMENT elem);
static RC set_hash_tri2(int hash_size, FEM_ELEMENT_FACE *hash,
                                               FEM_ELEMENT elem);
static RC set_hash_quad1(int hash_size, FEM_ELEMENT_FACE *hash,
                                               FEM_ELEMENT elem);
static RC set_hash_quad2(int hash_size, FEM_ELEMENT_FACE *hash,
                                               FEM_ELEMENT elem);
static RC set_hash_tetra1(int hash_size, FEM_ELEMENT_FACE *hash,
                                                FEM_ELEMENT elem);
static RC set_hash_tetra2(int hash_size, FEM_ELEMENT_FACE *hash,
                                                FEM_ELEMENT elem);
static RC set_hash_penta1(int hash_size, FEM_ELEMENT_FACE *hash,
                                                FEM_ELEMENT elem);
static RC set_hash_penta2(int hash_size, FEM_ELEMENT_FACE *hash,
                                                FEM_ELEMENT elem);
static RC set_hash_hexa1(int hash_size, FEM_ELEMENT_FACE *hash,
                                               FEM_ELEMENT elem);
static RC set_hash_hexa2(int hash_size, FEM_ELEMENT_FACE *hash,
                                               FEM_ELEMENT elem);
static RC set_cut_elem(FEM_ELEMENT_ARRAY *elem, FEM_ELEMENT_NODE_BOX box,
                       int index_size, const int *index);
static RC input_cut_elem_line1(FEM_ELEMENT_ARRAY *elem,
                                FEM_ELEMENT_NODE_BOX box);
static RC input_cut_elem_line2(FEM_ELEMENT_ARRAY *elem,
                                FEM_ELEMENT_NODE_BOX box);
static RC input_cut_elem_tri1(FEM_ELEMENT_ARRAY *elem,
                                FEM_ELEMENT_NODE_BOX box);
static RC input_cut_elem_tri2(FEM_ELEMENT_ARRAY *elem,
                                FEM_ELEMENT_NODE_BOX box);
static RC input_cut_elem_quad1(FEM_ELEMENT_ARRAY *elem,
                                FEM_ELEMENT_NODE_BOX box);
static RC input_cut_elem_quad2(FEM_ELEMENT_ARRAY *elem,
                                FEM_ELEMENT_NODE_BOX box);
static RC input_cut_elem_tetra1(FEM_ELEMENT_ARRAY *elem,
                                FEM_ELEMENT_NODE_BOX box);
static RC input_cut_elem_tetra2(FEM_ELEMENT_ARRAY *elem,
                                FEM_ELEMENT_NODE_BOX box);
static RC input_cut_elem_penta1(FEM_ELEMENT_ARRAY *elem,
                                FEM_ELEMENT_NODE_BOX box);
static RC input_cut_elem_penta2(FEM_ELEMENT_ARRAY *elem,
                                FEM_ELEMENT_NODE_BOX box);
static RC input_cut_elem_hexa1(FEM_ELEMENT_ARRAY *elem,
                               FEM_ELEMENT_NODE_BOX box);
static RC input_cut_elem_hexa2(FEM_ELEMENT_ARRAY *elem,
                               FEM_ELEMENT_NODE_BOX box);
static RC extract_edge(FEM_ELEMENT_ARRAY elem, int *edge_size,
                                               FEM_ELEMENT_EDGE **edge);
static int count_node_box_size(FEM_ELEM_TYPE type);
static int cal_edge_size(FEM_ELEMENT_ARRAY elem);
static RC interpolate_coord_edge(FEM_NODE_ARRAY *node, FEM_ELEMENT_ARRAY elem,
                                 FEM_ELEMENT_EDGE edge,
                                 FEM_ELEMENT_NODE_BOX *box, int *new_label);
static RC interpolate_coord_face(FEM_NODE_ARRAY *node, FEM_ELEMENT_ARRAY elem,
                                 FEM_ELEMENT_FACE face,
                                 FEM_ELEMENT_NODE_BOX *box, int *new_label);
static RC interpolate_coord_elem(FEM_NODE_ARRAY *node, FEM_ELEMENT_ARRAY elem,
                                 FEM_ELEMENT element,
                                 FEM_ELEMENT_NODE_BOX *box, int *new_label);
static RC local_inside_coord(FEM_ELEM_TYPE type, double **coord, int *number);
static RC cal_interpolate_node(FEM_NODE_ARRAY node, const double *N, 
                               int node_num, const int *node_label,
                               TRANSLATION3D *new_coord);
static RC add_node_array(FEM_NODE_ARRAY *node, TRANSLATION3D coord,
                                               int new_label);
static int check_rotation_edge(FEM_ELEM_TYPE type, int *index);
static RC set_edge(int hash_size, FEM_ELEMENT_EDGE *hash, int key_size,
                   const int *keys, int label, FEM_ELEM_TYPE type, int number);
static int elem_edge_index(int hash_size, FEM_ELEMENT_EDGE *hash,
                           int key_size, const int *keys);
static RC set_edge_tri1(int hash_size, FEM_ELEMENT_EDGE *hash,
                                        FEM_ELEMENT elem);
static RC set_edge_tri2(int hash_size, FEM_ELEMENT_EDGE *hash,
                                        FEM_ELEMENT elem);
static RC set_edge_quad1(int hash_size, FEM_ELEMENT_EDGE *hash,
                                        FEM_ELEMENT elem);
static RC set_edge_quad2(int hash_size, FEM_ELEMENT_EDGE *hash,
                                        FEM_ELEMENT elem);
static RC set_edge_tetra1(int hash_size, FEM_ELEMENT_EDGE *hash,
                                        FEM_ELEMENT elem);
static RC set_edge_tetra2(int hash_size, FEM_ELEMENT_EDGE *hash,
                                        FEM_ELEMENT elem);
static RC set_edge_penta1(int hash_size, FEM_ELEMENT_EDGE *hash,
                                        FEM_ELEMENT elem);
static RC set_edge_penta2(int hash_size, FEM_ELEMENT_EDGE *hash,
                                        FEM_ELEMENT elem);
static RC set_edge_hexa1(int hash_size, FEM_ELEMENT_EDGE *hash,
                                        FEM_ELEMENT elem);
static RC set_edge_hexa2(int hash_size, FEM_ELEMENT_EDGE *hash,
                                        FEM_ELEMENT elem);
...
{
	FEM_ELEMENT_EDGE *edge = NULL;
	FEM_ELEMENT_FACE *face = NULL;
	FEM_ELEMENT_NODE_BOX *box = NULL;
	int edge_size;
	int face_size;
	int box_size;
	int number;
	...
	box_size = elem->size;
	RC_NULL_CHK( box = malloc( box_size * sizeof(FEM_ELEMENT_NODE_BOX) ) );
	for(ii1=0; ii1<box_size; ii1++){
		...
		for(ii2=elem->array[ii1].node_num; ii2<FEM_CUT_MAX_NODE; ii2++){
			box[ii1].node[ii2] = -1;
		}
	}
	...
	RC_TRY( free_fem_element_array(elem) );
	RC_TRY( allocate_fem_element_array(0, elem) );
...
static void init_i_random(int hash_size)
{
	...
	my_random_init(I_RANDOM);
	for(ii1=0; ii1<I_RANDOM; ii1++){
		i_random[ii1] = (int)((double)hash_size * my_random());
...
static int elem_hash_index(int hash_size, FEM_ELEMENT_FACE *hash,
                                    int key_size, const int *keys)
{
	int skeys1[FEM_MAX_NODE_LIGHT];
	int skeys2[FEM_MAX_NODE_LIGHT];
	int ii1, ii2;
	int index;
	int inc;
...
static RC extract_face(FEM_ELEMENT_ARRAY elem, FEM_ELEMENT_ARRAY *face, int n)
{
	...
	RC_NULL_CHK( hash = malloc( hash_size * sizeof(FEM_ELEMENT_FACE) ) );
                    ...
                    int n, FEM_ELEMENT_ARRAY *face)
{
	...
	RC_TRY( allocate_fem_element_array(counter, face) );
	...
	for(ii1=0; ii1<hash_size; ii1++){
		if(hash[ii1].from_num == n){
			init_fem_element( &(face->array[face->size]) );
...
static RC clean_hash(FEM_ELEMENT_FACE *hash, int *hash_size)
{
	...
	*hash_size = 0;
	...
	hash = realloc(hash, (*hash_size) * sizeof(FEM_ELEMENT_FACE) );
	if(hash == NULL){
                       ...
                       int index_size, const int *index)
{
	...
	RC_TRY( realloc_fem_element_array(elem) );
                                               ...
                                               FEM_ELEMENT_EDGE **hash)
{
	...
	RC_NULL_CHK( (*hash) = malloc( hash_size * sizeof(FEM_ELEMENT_EDGE) ) );

	/* initialize hash */
	for(ii1=0; ii1<hash_size; ii1++){
		(*hash)[ii1].from_num = -1;
		for(ii2=0; ii2<FEM_EDGE_MAX_FROM; ii2++){
			(*hash)[ii1].from[ii2] = -1;
	...
	*edge_size = 0;
	...
	RC_NULL_CHK( (*hash)
	            = realloc((*hash), (*edge_size) * sizeof(FEM_ELEMENT_EDGE) ) );
                                 ...
                                 FEM_ELEMENT_NODE_BOX *box, int *new_label)
{
	...
	for(ii1=0; ii1<edge.from_num; ii1++){
		RC_NEG_CHK( index = search_fem_element_label(elem, edge.from[ii1]) );
								 ...
								 FEM_ELEMENT_NODE_BOX *box, int *new_label)
{
	...
	for(ii1=0; ii1<face.from_num; ii1++){
		RC_NEG_CHK( index = search_fem_element_label(elem, face.from[ii1]) );
		switch(face.type){
		case ELEM_TRI2:
			if(ii1 == 0){
								 ...
								 FEM_ELEMENT_NODE_BOX *box, int *new_label )
{
	...
	for(ii1=0; ii1<add_node_number; ii1++){
		...
		RC_NEG_CHK( index = search_fem_element_label(elem, element.label) );
							   ...
							   TRANSLATION3D *new_coord)
{
	...
	for(ii1=0; ii1<node_num; ii1++){
		RC_NEG_CHK( index = search_fem_node_label(node, node_label[ii1]) ); 
                   ...
                   const int *keys, int label, FEM_ELEM_TYPE type, int number)
{
	...
	hash[index].from_num ++;
	if( (hash[index].from_num >= FEM_EDGE_MAX_FROM)
	 || (hash[index].from_num < 0) ){
		fprintf(stderr, "set_edge()(from_num = %d) < \n", hash[index].from_num);
		return(OVERFLOW_ERROR_RC);
	}	
	hash[index].from[ hash[index].from_num - 1 ] = label;
