...
/* $Id: nst_pch_component.c,v 1.6 2003/07/23 04:06:31 sasaoka Exp $ */
...
static RC seek_pch(FILE *fp, int *pch_data, int *subcase,
...
static void idc2ss_3d(const IDC idc1[], const IDC idc2[], const IDC idc3[],
                      ...
                      int node_id, FEM_STRESS_STRAIN *ss);
               ...
               FEM_STRAIN_ARRAY *ss)
{
	...
	RC_TRY( allocate_fem_strain_array(INIT_SIZE, ss) );
		  ...
		  ||(tmp_elem_type == ELEM_HEXA1) ){
			while(1){
				...
				for(ii1=-1; ii1<node_num; ii1++){
					...
					RC_TRY( realloc_fem_strain_array(ss) );
	...
	RC_TRY( clean_fem_strain_array(ss) );
	return(NORMAL_RC);
}
