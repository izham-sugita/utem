...
/* $Id: glut_utl.c 864 2007-01-17 10:02:04Z sasaoka $ */
...
static void drawObject(ELEMENT_ARRAY elem, NODE_ARRAY node);
...
static void drow_fem_model(ELEMENT_ARRAY elem, NODE_ARRAY node); 
static RC initialize_object(NODE_ARRAY node);
RC show_fem_model(ELEMENT_ARRAY elem, NODE_ARRAY node);
...
static VECT3D max_object_size = {0.0, 0.0, 0.0};
static VECT3D min_object_size = {0.0, 0.0, 0.0};
...
static ELEMENT_ARRAY elem;
static GLint drowtype = GL_LINE_LOOP; /* デフォルト描画タイプ：POLYGON */

RC
show_fem_model (ELEMENT_ARRAY elem_org, NODE_ARRAY node_org)
{
...
static void
drow_fem_model (ELEMENT_ARRAY elem, NODE_ARRAY node)
{
	int ii1,ii2;
	int index;
...
/* オブジェクトサイズを設定 */
static RC
initialize_object (NODE_ARRAY node)
{
	int ii1;
	VECT3D center;
	double length;
...
static void
drawObject (ELEMENT_ARRAY elem, NODE_ARRAY node)
{
...
static void
sp_keyboard (int key, int x,  int y)
{
	switch(key){
	case GLUT_KEY_LEFT:
		spin_x += 5;
