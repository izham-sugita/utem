...
/* $Id: flow_solver.c 842 2006-12-04 06:21:20Z sasaoka $ */
...
/* 普通の ABS_ERROR でよければそれに変更する．互換のため置換しておく． */
#define FL_ABS_ERROR (10.0*ABS_ERROR) /* 絶対誤差 */

typedef struct{
	int num_point;
	double **g_points;
	double N[MAX_GAUSS_POINT][MAX_NODE];
...
static RC solve_middle_velocity(double delta_t, double Re, 
                                ...
                                BC_ARRAY bc_velo, VELO_ARRAY vp,
                                VELO_ARRAY *vp1);
static RC set_f_vector_for_pressure(int dim, double delta_t,
                                    NODE_ARRAY node,
                                    ELEMENT_ARRAY element,
                                    VELO_ARRAY vp1, double *f_vector);
static RC set_elem_matrix_for_pressure(ELEMENT element, NODE_ARRAY node,
                                       int *index_array, int *matrix_size,
                                       double **elem_matrix);
static RC solve_pressure(int dim, double delta_t, NODE_ARRAY node,
                         ELEMENT_ARRAY element, VELO_ARRAY *vp1, 
                         BC_ARRAY bc);
static RC solve_velocity(double delta_t, NODE_ARRAY node,
                         ELEMENT_ARRAY element, BC_ARRAY bc_velo,
                         VELO_ARRAY vp1, VELO_ARRAY *vp);


/* 随伴解析---------------------------------------------------------------*/
static RC set_f_vector_for_adjoint_analysis(int dim, NODE_ARRAY node, 
                                            ELEMENT_ARRAY element,
                                            BC_ARRAY bc, VELO_ARRAY vp,
                                            double *f_vector);
...
/* 感度導出用ツール------------------------------------------------------ */
static RC local_velocity_strain(ELEMENT element, const double *local_point,
                                NODE_ARRAY node, VELO_ARRAY velo,
                                STRESS_STRAIN *strain_velo);
static RC fill_velo_vector(ELEMENT element, VELO_ARRAY velo,
                           double *velo_v);


/* 解析ツール------------------------------------------------------------ */
static RC allocate_g_prop(G_POINT_PROP *g_prop);
static RC set_g_prop(ELEMENT element, NODE_ARRAY node,
                     G_POINT_PROP *g_prop);
static RC allocate_matrix(int mode, int dim, NODE_ARRAY node,
                          ELEMENT_ARRAY element,
                          SKYLINE_MATRIX *g_matrix);
static RC flow_modify_matrix(int mode, NODE_ARRAY node, 
                             BC_ARRAY bc_press,
                             SKYLINE_MATRIX g_matrix, double *f_vector);
...
static RC imp_g_matrix(int mode, int matrix_size, const int *index_array,
                       double **elem_matrix, SKYLINE_MATRIX g_matrix);
static RC set_Mass_matrix(int node_num, G_POINT_PROP g_prop, double **M);
static RC set_K_matrix(int dN_xyz, int v_xyz, int dim, NODE_ARRAY node,
                       ELEMENT element, G_POINT_PROP g_prop, double **V,
                       double **K);
static RC set_S_matrix(int dN_xyz1, int dN_xyz2, int dim, ELEMENT element, 
                       G_POINT_PROP g_prop, double **S);
static RC set_H_matrix(int dN_xyz, ELEMENT element, G_POINT_PROP g_prop,
                       double **H);
static RC set_H1_matrix(int dN_xyz, ELEMENT v_elem, ELEMENT p_elem,
                        G_POINT_PROP v_g_prop, G_POINT_PROP p_g_prop,
                        double **H1);
static RC set_H2_matrix(int dN_xyz, ELEMENT v_elem, ELEMENT p_elem,
                        G_POINT_PROP v_g_prop, G_POINT_PROP p_g_prop,
                        double **H2);
static RC set_advection_term(int dim, NODE_ARRAY node, ELEMENT element,
                             G_POINT_PROP g_prop, double **V, double **ADV);
static RC set_viscosity_term(int dim,  ELEMENT element, G_POINT_PROP g_prop,
                             double **V, double **VIS);
static RC make_press_element_data(ELEMENT_ARRAY velo_elem,
                                  ELEMENT_ARRAY *press_elem);
static RC pivotting_check(NODE_ARRAY node, SKYLINE_MATRIX g_matrix,
                          double *vector);
static RC set_Gauss_const(ELEM_TYPE type, G_POINT_PROP *g_prop);
static RC free_flow_g_matrix(int mode, SKYLINE_MATRIX *g_matrix);
static RC allocate_g_prop(G_POINT_PROP *g_prop);
static RC flow_property_print(int dim, int iteration, double Re,
                              double delta_t);


static RC modify_velo_array(BC_ARRAY bc_velo, BC_ARRAY bc_press,
                            VELO_ARRAY *flow);
static RC modify_nonzero_matrix1a(BC_ARRAY bc_velo, BC_ARRAY bc_press,
                                  NODE_ARRAY node, int valid_node_num,
...
static RC modify_nonzero_matrix1a_diagonal(int diagonal_index, double value,
                                           ...
                                           double *global_vect);
static RC impose_nonzero1a(ELEM_MATRIX elem_matrix,
                           const double *local_vect,
                           NONZERO_MATRIX *matrix, double *global_vect);
static RC make_flow_elem_matrix(int dim, int model_order, ELEMENT elem,
                                NODE_ARRAY node, int valid_node_num,
                                int velo_order, int press_order,
                                double density, double viscosity,
                                const double *flow_vect, 
                                ELEM_MATRIX *elem_matrix, double **local_vect,
                                WORK_SET ws);
static RC velo2vect(int dim, NODE_ARRAY node, VELO_ARRAY flow,
                    int valid_node_num, double *flow_vect);
static RC vect2velo(int dim, NODE_ARRAY node, ELEMENT_ARRAY elem,
                    int velo_order, int press_order, int valid_node_num,
                    const double *flow_vect, VELO_ARRAY *flow);
static RC parameter_check(ELEMENT elem, int model_order, int para_order,
                          ELEM_TYPE *para_type, int *para_node_num);

static RC make_adjoint_elem_matrix(int dim, int model_order, ELEMENT elem,
                                   NODE_ARRAY node, int valid_node_num,
                                   int velo_order, int press_order,
                                   double density, double viscosity,
                                   const double *flow_vect,
                                   ELEM_MATRIX *elem_matrix,
                                   double **local_vect, WORK_SET ws);
 ...
 */
RC
steady_flow_analysis (NODE_ARRAY node, ELEMENT_ARRAY elem,
                      BC_ARRAY bc_velo, BC_ARRAY bc_press,
                      int velo_order, int press_order,
                      double density, double viscosity, VELO_ARRAY *flow)
{
	int ii1;
	int dim;
	int model_order;
	...
	dim = analysis_dim(elem);
	if( (dim != 2) && (dim != 3) ) return(IMPLEMENT_ERROR_RC);

	model_order = analysis_order(elem);

	if(model_order != 2) return(IMPLEMENT_ERROR_RC);
	...
	if( (flow->size != node.size) && (flow->source != FEM_K_SOL) ){
		init_flag = 1;
		/* VELO_ARRAY の確保 */
		RC_TRY( allocate_velo_array(node.size, flow) );
	...
	RC_TRY( log_printf(4, "Making Matrix........") );
		...
		for(ii1=0; ii1<elem.size; ii1++){
			...
			if(init_flag == 1){
				RC_TRY( make_flow_elem_matrix(dim, model_order,
				            elem.array[ii1], node, valid_node_num, velo_order,
				            press_order, 0.0, viscosity, flow_vect,
				            &elem_matrix, &local_vect, ws) );
			}else{
				RC_TRY( make_flow_elem_matrix( dim, model_order,
				            elem.array[ii1], node, valid_node_num, velo_order,
				            press_order, density, viscosity, flow_vect,
				            &elem_matrix, &local_vect, ws) );
			}

			RC_TRY( impose_nonzero1a(elem_matrix, local_vect, &matrix,
			                         global_vect) );

			RC_TRY( free_elem_matrix(&elem_matrix) );
			RC_TRY( mm_free( (void *)local_vect ) );
		}
		init_flag = 0;
		...
		RC_TRY( log_printf(4, "   norm = %e\n", delta_max) );

		RC_TRY( free_nonzero_matrix1a(&matrix) );
		RC_TRY( mm_free( (void *)global_vect ) );

		/* Newton-Raphson の収束 */
		if(delta_max < NR_REL_ERROR) break;

		/* Stokes 流れは線形連立方程式 */
		if(density < FL_ABS_ERROR) break;
	}

	RC_TRY( vect2velo(dim, node, elem, velo_order, press_order,
	                  valid_node_num, flow_vect, flow) );

	RC_TRY( free_work_set(&ws) );
	RC_TRY( mm_free( (void *)flow_vect ) );
	RC_TRY( mm_free( (void *)delta ) );
...
static RC
modify_velo_array (BC_ARRAY bc_velo, BC_ARRAY bc_press, VELO_ARRAY *flow)
{
	int index;
	int ii1;
	...
	for(ii1=0; ii1<bc_press.size; ii1++){
		...
		index = search_velo_node(*flow, bc_press.array[ii1].node);
		RC_NEG_CHK(index);
	...
	for(ii1=0; ii1<bc_velo.size; ii1++){
		...
		index = search_velo_node(*flow, bc_velo.array[ii1].node);
		RC_NEG_CHK(index);
...
static RC
modify_nonzero_matrix1a (BC_ARRAY bc_velo, BC_ARRAY bc_press,
                         NODE_ARRAY node, int valid_node_num,
                         NONZERO_MATRIX *matrix, double *global_vect)
{
	int tmp;
	...
	for(ii1=0; ii1<bc_press.size; ii1++){
		...
		index = search_node_label(node, bc_press.array[ii1].node);
		RC_NEG_CHK(index);
	...
	for(ii1=0; ii1<bc_velo.size; ii1++){
		...
		index = search_node_label(node, bc_velo.array[ii1].node);
		RC_NEG_CHK(index);
		tmp = node.array[index].renum;
...
static RC
modify_nonzero_matrix1a_diagonal (int diagonal_index, double value, 
                                  NONZERO_MATRIX *matrix, double *global_vect)
{
	int ii1, ii2;
...
static RC
impose_nonzero1a (ELEM_MATRIX elem_matrix, const double *local_vect,
                  NONZERO_MATRIX *matrix, double *global_vect)
{
	double *ptr;
	int ii1, ii2;
...
static RC
make_flow_elem_matrix (int dim, int model_order, ELEMENT elem, NODE_ARRAY node,
                       int valid_node_num, int velo_order, int press_order,
                       double density, double viscosity,
                       const double *flow_vect, ELEM_MATRIX *elem_matrix,
                       double **local_vect, WORK_SET ws)
{
	...
	if(allocate_flag == 0){
		RC_TRY( allocate3D(MAX_NODE, MAX_NODE, MAX_NODE,
		                   &Kx_tensor) );
		RC_TRY( allocate3D(MAX_NODE, MAX_NODE, MAX_NODE,
		                   &Ky_tensor) );

		RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &Sxx_matrix) );
		RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &Sxy_matrix) );
		RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &Syy_matrix) );
		RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &Hx_matrix) );
		RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &Hy_matrix) );

		if(dim == 3){
			RC_TRY( allocate3D(MAX_NODE, MAX_NODE, MAX_NODE,
		                       &Kz_tensor) );
			RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &Szz_matrix) );
			RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &Syz_matrix) );
			RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &Szx_matrix) );
			RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &Hz_matrix) );
		}
	...
	for(ii1=0; ii1<MAX_NODE; ii1++){
		 init_matrix(MAX_NODE, MAX_NODE, Kx_tensor[ii1]);
		 init_matrix(MAX_NODE, MAX_NODE, Ky_tensor[ii1]);
	}
	init_matrix(MAX_NODE, MAX_NODE, Sxx_matrix);
	init_matrix(MAX_NODE, MAX_NODE, Syy_matrix);
	init_matrix(MAX_NODE, MAX_NODE, Sxy_matrix);
	init_matrix(MAX_NODE, MAX_NODE, Hx_matrix);
	init_matrix(MAX_NODE, MAX_NODE, Hy_matrix);
	if(dim == 3){
		init_matrix(MAX_NODE, MAX_NODE, Szz_matrix);
		init_matrix(MAX_NODE, MAX_NODE, Syz_matrix);
		init_matrix(MAX_NODE, MAX_NODE, Szx_matrix);
		init_matrix(MAX_NODE, MAX_NODE, Hz_matrix);
		for(ii1=0; ii1<MAX_NODE; ii1++){
			init_matrix(MAX_NODE, MAX_NODE, Kz_tensor[ii1]);
		}
	}

	base_type = elem.type;
	base_node_num = elem.node_num;
	RC_TRY( parameter_check(elem, model_order, velo_order,
	                        &velo_type, &velo_node_num) );
	RC_TRY( parameter_check(elem, model_order, press_order,
	...
	for(ii1=0; ii1<g_point_num; ii1++){
		...
		if(det_J < 0){
			RC_TRY( warning_printf(30001, elem.label) );
	...
	elem_matrix->index = mm_alloc( elem_matrix->size * sizeof(int) );
	...
	RC_TRY( mm_free((void *)elem_flow_vect) );
...
static RC
velo2vect (int dim, NODE_ARRAY node, VELO_ARRAY flow,
           int valid_node_num, double *flow_vect)
{
	int index;
	int ii1;
...
static RC
vect2velo (int dim, NODE_ARRAY node, ELEMENT_ARRAY elem,
           int velo_order, int press_order, int valid_node_num,
           const double *flow_vect, VELO_ARRAY *flow)
{
	int ii1, ii2, ii3;
	...
	RC_TRY( allocate2D(MAX_NODE, 4, &local_coords) );
	RC_TRY( free_velo_array(flow) );
	RC_TRY( allocate_velo_array(node.size, flow) );
	...
	if(press_order == 1){
		for(ii1=0; ii1<elem.size; ii1++){
			...
			for(ii2=press_node_num; ii2<elem.array[ii1].node_num; ii2++){ 
				...
				value[0] = 0.0;
				for(ii3=0; ii3<press_node_num; ii3++){
					index = search_velo_node(*flow,
					                             elem.array[ii1].node[ii3]);
					RC_NEG_CHK(index);
					value[0] += N[ii3] * flow->array[index].s;
				}

				index = search_velo_node(*flow, elem.array[ii1].node[ii2]);
				RC_NEG_CHK(index);
	...
	if(velo_order == 1){
		for(ii1=0; ii1<elem.size; ii1++){
			...
			for(ii2=velo_node_num; ii2<elem.array[ii1].node_num; ii2++){ 
				...
				value[2] = 0.0;
				for(ii3=0; ii3<velo_node_num; ii3++){
					index = search_velo_node(*flow, elem.array[ii1].node[ii3]);
					RC_NEG_CHK(index);
	...
	RC_TRY( free2D(MAX_NODE, 4, &local_coords) );
...
static RC
parameter_check (ELEMENT elem, int model_order, int para_order,
                 ELEM_TYPE *para_type, int *para_node_num)
{
...
RC
steady_adjflow_analysis (NODE_ARRAY node, ELEMENT_ARRAY elem,
                         BC_ARRAY bc_velo, BC_ARRAY bc_press,
                         int velo_order, int press_order,
                         double density, double viscosity,
                         VELO_ARRAY flow, VELO_ARRAY *adjoint_flow)
{
	...
	/* VELO_ARRAY の確保 */
	RC_TRY( allocate_velo_array(node.size, adjoint_flow) );
	...
	RC_TRY( mm_free( (void *)adjoint_flow_vect ) );
	RC_TRY( free_work_set(&ws) );
...
/* 前半部は make_flow_elem_matrix() と共通部分が多いため，モジュール化可能 */
static RC
make_adjoint_elem_matrix (int dim, int model_order, ELEMENT elem,
                          NODE_ARRAY node, int valid_node_num, int velo_order,
                          int press_order, double density, double viscosity,
                          const double *flow_vect, ELEM_MATRIX *elem_matrix,
                          double **local_vect, WORK_SET ws)
{
	static int allocate_flag = 0;
...
RC
flow_solve_for_optimization (double delta_t, int iteration, double Re, 
                             NODE_ARRAY node, ELEMENT_ARRAY element, 
                             BC_ARRAY bc_velo, BC_ARRAY bc_press,
                             VELO_ARRAY *vp)
{
	...
	RC_TRY( allocate_velo_array(node.size, &tmp_vp1) );
	RC_TRY( allocate_velo_array(node.size, &tmp_vp2) );
	RC_TRY( allocate_velo_array(node.size, vp) );
	...
	RC_TRY( free_velo_array(&tmp_vp1) );
	RC_TRY( free_velo_array(&tmp_vp2) );
...
RC
flow_solve_non_steady_flow (int plot_step, double delta_t, int iteration,
                            double Re, NODE_ARRAY node, ELEMENT_ARRAY element,
                            BC_ARRAY bc_velo, BC_ARRAY bc_press,
                            VELO_ARRAY *vp)
{
	int ii1, ii2;
	...
	for(ii1=0; ii1<iteration; ii1++){
		...
		RC_TRY( log_printf(4, "%d : norm = %e ", ii1,
		                   ( sqrt(delta_norm)/sqrt(norm) )/node.size ) );
...
/* 中間流速の計算 */
static RC
solve_middle_velocity (double delta_t, double Re,
                       NODE_ARRAY node, ELEMENT_ARRAY element,
                       BC_ARRAY bc_velo, VELO_ARRAY vp, VELO_ARRAY *vp1)
{
	int dim = 0;
	int index;
	int ii1, ii2;
	...
	if(init_flag == 0){
		RC_TRY( allocate2D(3, MAX_NODE, &ADV) );
		RC_TRY( allocate2D(3, MAX_NODE, &VIS) );
		RC_TRY( allocate2D(3, MAX_NODE, &V1) );
		RC_TRY( allocate2D(3, node.size, &V2) );
		RC_TRY( allocate_g_prop(&g_prop) );
		AMB = (double *)mm_alloc(node.size * sizeof(double));
		if(AMB == NULL) return(ALLOC_ERROR_RC);
		init_flag = 1;
	}
	...
	for(ii1=0; ii1<element.size; ii1++){
		...
		for(ii2=0; ii2<element.array[ii1].node_num; ii2++){
			index = search_node_label(node, element.array[ii1].node[ii2]);
			RC_NEG_CHK(index);
			AMB[index] += 1.0;
	...
	for(ii1=0; ii1<node.size; ii1++){
		if(AMB[ii1] < FL_ABS_ERROR) return(CAL_ERROR_RC);
		SS = delta_t / AMB[ii1];
...
/* 対流項の計算 */
static RC
set_advection_term (int dim, NODE_ARRAY node, ELEMENT element, 
                    G_POINT_PROP g_prop, double **V, double **ADV)
{
	int ii1, ii2, ii3, ii4;
	static int init_flag = 0;
	...
	if(init_flag == 0){
		RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &K) );
		RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &M) );
		RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &inverse_M) );
		RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &inverse_MK) );
		init_flag = 1;
	}
...
/* 粘性項の計算 */
static RC
set_viscosity_term (int dim, ELEMENT element, G_POINT_PROP g_prop, 
                    double **V, double **VIS)
{
	int ii1, ii2, ii3, ii4;
	static int init_flag = 0;
	...
	if(init_flag == 0){
		RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &S) );
		RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &M) );
		RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &inverse_M) );
		RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &inverse_MS) );
		init_flag = 1;
	}
...
/* ポアソン方程式右辺（荷重項）の計算 */
static RC
set_f_vector_for_pressure (int dim, double delta_t, 
                           NODE_ARRAY node, ELEMENT_ARRAY element, 
                           VELO_ARRAY vp1, double *f_vector)
{
	int index1, index2;
	...
	if(init_flag == 0){
		RC_TRY( allocate2D(3, MAX_NODE, &V) );
		RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &H) );
		RC_TRY( allocate_g_prop(&g_prop) );
		init_flag = 1;
	}
	...
	for(ii1=0; ii1<element.size; ii1++){
		...
		for(ii2=0; ii2<element.array[ii1].node_num; ii2++){
			index1 = search_node_label(node, element.array[ii1].node[ii2]);
			RC_NEG_CHK(index1);
...
static RC
set_elem_matrix_for_pressure (ELEMENT element, NODE_ARRAY node,
                              int *index_array, int *matrix_size,
                              double **elem_matrix)
{
	int dim = 0;
	...
	if(init_flag == 0){
		RC_TRY( allocate2D(dim, MAX_NODE, &dNxy) );
		RC_TRY( allocate_g_prop(&g_prop) );
		init_flag = 1;
	}
	...
	for(ii1=0; ii1<element.node_num; ii1++){
		...
		RC_NEG_CHK( index = search_node_label(node, element.node[ii1]) );
...
/* 圧力の計算 */
static RC
solve_pressure (int dim, double delta_t, NODE_ARRAY node,
                ELEMENT_ARRAY element, VELO_ARRAY *vp1, BC_ARRAY bc_press)
{
	int ii1;
	int matrix_size;
	int index_array[MAX_NODE];
	double *f_vector;
	...
	if(init_flag == 0){
		RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &elem_matrix) );
		init_flag = 1;
	}

	RC_NEG_ZERO_CHK(node.size);
	RC_NULL_CHK( f_vector = (double *)mm_alloc( node.size*sizeof(double) ) );
	...
	RC_TRY( mm_free( (void *)f_vector ) );

	RC_TRY( free_flow_g_matrix(1, &g_matrix) );
...
/* 流速の計算 */
static RC
solve_velocity (double delta_t, NODE_ARRAY node, ELEMENT_ARRAY element,
                BC_ARRAY bc_velo, VELO_ARRAY vp1, VELO_ARRAY *vp)
{
	int dim = 0;
	...
	if(init_flag == 0){
		...
		RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &M) );
		RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &inverse_M) );
		RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &inverse_MH) );
		RC_TRY( allocate_g_prop(&g_prop) );
		AMB = (double *)mm_alloc(node.size * sizeof(double));
		if(AMB == NULL) return(ALLOC_ERROR_RC);
		init_flag = 1;
	...
	for(ii1=0; ii1<element.size; ii1++){
		...
		for(ii2=0; ii2<dim; ii2++){
			RC_TRY( set_H_matrix(ii2, element.array[ii1], g_prop, H) );
			mul_matrix(element.array[ii1].node_num,
		...
		for(ii2=0; ii2<element.array[ii1].node_num; ii2++){
			index3 = search_node_label(node, element.array[ii1].node[ii2]);
			RC_NEG_CHK(index3);
			AMB[index3] += 1.0;
		}
	}
	...
	/* 流速の境界条件 */
	for(ii1=0; ii1<bc_velo.size; ii1++){
		index3 = search_node_label(node, bc_velo.array[ii1].node);
		RC_NEG_CHK(index3);
		if(bc_velo.array[ii1].v_type.x == BC_VELO){
...
RC
flow_adjoint_analysis (double Re, NODE_ARRAY node, ELEMENT_ARRAY element,
                       BC_ARRAY bc, VELO_ARRAY vp, VELO_ARRAY *adjoint_vp)
{
	int ii1;
	int dim;
	int matrix_size;
	int index_array[4*MAX_NODE];
	double *f_vector;
	static double **elem_matrix = NULL;
	int p_flag = 1;
	static int init_flag = 0;
	SKYLINE_MATRIX g_matrix;
	ELEMENT_ARRAY press_element;
	FILE *fp;
	...
	if(init_flag == 0){
		RC_TRY( allocate2D(4*MAX_NODE, 4*MAX_NODE, &elem_matrix) );
		RC_TRY( allocate_element_array(element.size, &press_element) );
		RC_TRY( allocate_velo_array(node.size, adjoint_vp) );
		init_flag = 1;
	}

	fp = fopen("./test2.dat","w");
	if( fp == NULL) return(OPEN_ERROR_RC);
	...
	for(ii1=0; ii1<element.size; ii1++){
		if(element.array[ii1].label < 0) continue;
		RC_TRY( set_elem_matrix_for_adjoint_analysis(
		            dim, Re, element.array[ii1], press_element.array[ii1],
		            node, vp, index_array, &matrix_size, elem_matrix) );
	...
	RC_TRY( free_flow_g_matrix(2, &g_matrix) );
	fclose(fp);
	RC_TRY( mm_free(f_vector) );
...
/* 流速・圧力パラメータより随伴解析を行い、随伴変数を渡す */
RC
flow_adjoint_analysis2 (double Re, NODE_ARRAY node, ELEMENT_ARRAY element,
                        BC_ARRAY bc, VELO_ARRAY vp, VELO_ARRAY *adjoint_vp)
{
	int ii1, ii2, ii3;
	int dim;
	...
	if(init_flag == 0){
		index = (int *)mm_alloc(4*node.size * sizeof(int));
		if(index == NULL) return(ALLOC_ERROR_RC);
...
/* 2次元のみ */
static RC
set_f_vector_for_adjoint_analysis (int dim, NODE_ARRAY node, 
                                   ELEMENT_ARRAY element, BC_ARRAY bc,
                                   VELO_ARRAY vp, double *f_vector)
{
	int ii1, ii2, ii3;
	int index1, index2;
	...
	for(ii1=0; ii1<element.size; ii1++){
		...
		if( (check_flag > 1) && (check_param > FL_ABS_ERROR) && (dim == 2) ){
			index1 = search_node_label(node, check_index[0]);
			index2 = search_node_label(node, check_index[1]);
			bc_index1 =  search_bc_node(bc, check_index[0]);
			bc_index2 =  search_bc_node(bc, check_index[1]);
			...
			RC_TRY( log_printf(4, "%d %d : \n",
			                   check_index[0] ,check_index[1]) );
			RC_TRY( log_printf(4, "%f %f %f %f\n", AJ, BJ, Up, Vp) );
		}
	}
...
static RC
set_elem_matrix_for_adjoint_analysis (int dim, double Re, ELEMENT v_element,
                                      ELEMENT p_element, NODE_ARRAY node,
                                      VELO_ARRAY vp, int *index_array,
                                      int *matrix_size, double **elem_matrix)
{
	int ii1, ii2, ii3, ii4;
	int g_dim = 0;
	int n_num = 0;
	...
	if(init_flag == 0){
		...
		RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &S) );
		RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &H) );
		RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &H1) );
		RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &H2) );
	...
	for(ii1=0; ii1 < v_element.node_num; ii1++){
		int index;
		RC_NEG_CHK( index = search_node_label(node, v_element.node[ii1]) );
...
static RC
adjoint_modify_matrix (int dim, int matrix_size, NODE_ARRAY node, BC_ARRAY bc,
                       double **array, double *vector)
{
	int ii1, ii2;
	int index;
...
/* ひずみ速度の計算 */
RC
strain_velocity (int sol_ss_flag, NODE_ARRAY node, ELEMENT_ARRAY element,
                 VELO_ARRAY velo, STRAIN_ARRAY *strain_velo)
{
	int ii1, ii2;
	static int init_flag = 0;
	...
	RC_TRY( allocate_strain_array(0, strain_velo) );
	...
	for(ii1=0; ii1<element.size; ii1++){
		...
		if(sol_ss_flag & SOL_SS_CENTER){
			RC_TRY( realloc_strain_array(strain_velo) );
	...
	RC_TRY( clean_strain_array(strain_velo) );
...
/* 要素の局所ひずみ速度 */
static RC
local_velocity_strain (ELEMENT element, const double *local_point,
                       NODE_ARRAY node, VELO_ARRAY velo,
                       STRESS_STRAIN *strain_velo)
{
	int dim, ssnum;
	double velo_v[3*MAX_NODE];
	...
	init_stress_strain(strain_velo);
...
static RC
fill_velo_vector (ELEMENT element, VELO_ARRAY velo, double *velo_v)
{
	int dim;
	int ii1;
	...
	for(ii1=0; ii1<element.node_num; ii1++){
		...
		index = search_velo_node(velo, element.node[ii1]);
		RC_NEG_CHK(index);
...
/* 全境界節点から壁面境界節点を取り出す */
RC
pick_out_wall_bc_node (BC_ARRAY bc, NODE_ARRAY *wall_node)
{
	int ii1;
	static int velo_flag = 0;

	RC_TRY( allocate_node_array(0, wall_node) );
	wall_node->size = 0;
	...
	for(ii1=0;ii1<bc.size;ii1++){
		...
		if( (bc.array[ii1].v_type.x == BC_VELO)
		 && (bc.array[ii1].v.x > FL_ABS_ERROR) ){
			velo_flag = 1;
		}
		if( (bc.array[ii1].v_type.y == BC_VELO)
		 && (bc.array[ii1].v.y > FL_ABS_ERROR) ){
			velo_flag = 1;
		}
		if( (bc.array[ii1].v_type.z == BC_VELO)
		 && (bc.array[ii1].v.z > FL_ABS_ERROR) ){
			velo_flag = 1;
		}

		if( (velo_flag != 1) && (bc.array[ii1].s_type[0] == BC_FREE) ){
			RC_TRY( realloc_node_array(wall_node) );
 ...
 */
RC
change_bc_type4flow (BC_ARRAY *bc)
{
	int ii1;
...
/* mode : 1. コレスキー　2. クラウト */
static RC
free_flow_g_matrix (int mode, SKYLINE_MATRIX *g_matrix)
{
	switch(mode){
	case 1:
...
/* G_PROPへ各種値を格納 */
static RC
set_g_prop (ELEMENT element, NODE_ARRAY node, G_POINT_PROP *g_prop)
{
	int ii1, ii2;
	static double **node_location = NULL;
...
static RC
flow_modify_matrix (int mode, NODE_ARRAY node, BC_ARRAY bc_press,
                    SKYLINE_MATRIX g_matrix, double *f_vector)
{
	int ii1;
	int index;
...
/* mode : 1. コレスキー, 2. クラウト */
static RC
allocate_matrix (int mode, int dim, NODE_ARRAY node, ELEMENT_ARRAY element,
                 SKYLINE_MATRIX *g_matrix)
{
...
fill_index1 (int dim, NODE_ARRAY node, ELEMENT_ARRAY element, long *index1)
{
	...
	for(ii1=0; ii1<element.size; ii1++){
		...
		for(ii2=0; ii2<element.array[ii1].node_num; ii2++){
			pos1 = dim * search_node_label(node,
			                               element.array[ii1].node[ii2]);
			RC_NEG_CHK(pos1);
			for(ii3=0; ii3<element.array[ii1].node_num; ii3++){
				pos2 = dim * search_node_label(node,
				                               element.array[ii1].node[ii3]);
				RC_NEG_CHK(pos2);
...
static RC
imp_g_matrix (int mode, int matrix_size, const int *index_array,
              double **elem_matrix, SKYLINE_MATRIX g_matrix)
{
	RC rc;
	int ii1, ii2;
...
/* 圧力用要素データの作成 */
static RC
make_press_element_data (ELEMENT_ARRAY velo_elem, ELEMENT_ARRAY *press_elem)
{
	int ii1, ii2;
...
static RC
pivotting_check (NODE_ARRAY node, SKYLINE_MATRIX g_matrix, double *vector)
{
	...
	for(ii1=0;ii1<node.size;ii1++){
		for(ii2=0;ii2<g_dim;ii2++){
			if( fabs(g_matrix.array[g_matrix.index2[ii1*g_dim+ii2]])
			    < FL_ABS_ERROR){
				get_param = 0;
				...
				if(fabs(get_param) < FL_ABS_ERROR){
					RC_TRY( log_printf(4, "%d %d\n ", ii1+1, ii2+1) );

					for(ii4=0;ii4<g_dim;ii4++){
						for(ii5=0;ii5<g_dim;ii5++){
							RC_TRY( log_printf(4, "%f ",
							    s_crout_get(g_matrix.dof, g_matrix.index1,
							    g_matrix.index2, g_matrix.array,
								ii1*g_dim+ii4, ii1*g_dim+ii5, &rc)) );
						}
						RC_TRY( log_printf(4, "\n ") );
					}
					RC_TRY( log_printf(4, " pivotting error\n") );
					/* return(CAL_ERROR_RC); */
				}
			}
		}
...
/* 質量行列の計算 */
static RC
set_Mass_matrix (int node_num, G_POINT_PROP g_prop, double **M)
{
	int ii1, ii2, ii3;
...
/* 移流項行列の計算 */
static RC
set_K_matrix (int dN_xyz, int v_xyz, int dim, NODE_ARRAY node,
              ELEMENT element, G_POINT_PROP g_prop, double **V, double **K)
{
	int ii1, ii2, ii3;
	double sum_dNV;
	static double **dN;
	...
	if(init_flag == 0){
		RC_TRY( allocate2D(MAX_NODE, MAX_NODE, &NN) );
		RC_TRY( allocate2D(3, MAX_NODE, &dN) );
		init_flag = 1;
	}
...
/* 粘性項行列の計算 */
static RC
set_S_matrix (int dN_xyz1, int dN_xyz2, int dim, ELEMENT element, 
              G_POINT_PROP g_prop, double **S)
{
	int ii1, ii2, ii3;
	static double **dNxy;
...
/* ポアソン方程式右辺行列の計算 */
static RC
set_H_matrix (int dN_xyz, ELEMENT element, G_POINT_PROP g_prop, double **H)
{
	int ii1, ii2, ii3;
	static double **dNxy;
	static int init_flag = 0;
	...
	if(init_flag == 0){
		RC_TRY( allocate2D(3, MAX_NODE, &dNxy) );
		init_flag = 1;
	}
...
static RC
set_H1_matrix (int dN_xyz, ELEMENT v_elem, ELEMENT p_elem,
               G_POINT_PROP v_g_prop, G_POINT_PROP p_g_prop, double **H1)
{
	int ii1, ii2, ii3;
	static double **dNxy;
	static int init_flag = 0;
...
static RC
set_H2_matrix (int dN_xyz, ELEMENT v_elem, ELEMENT p_elem,
               G_POINT_PROP v_g_prop, G_POINT_PROP p_g_prop, double **H2)
{
	int ii1, ii2, ii3;
	static double **dNxy;
	static int init_flag = 0;
...
static RC
set_Gauss_const (ELEM_TYPE type, G_POINT_PROP *g_prop)
{
	switch(type){
	case ELEM_TRI1:
		g_prop->num_point = 4;
...
/* G_PROP の確保 */
static RC
allocate_g_prop (G_POINT_PROP *g_prop)
{
	int ii1;
	...
	for(ii1=0; ii1<MAX_GAUSS_POINT; ii1++){
		RC_TRY( allocate2D(3, MAX_NODE, &(g_prop->dNxy[ii1]) ) );
	}
...
static RC
flow_property_print (int dim, int iteration, double Re, double delta_t)
{
