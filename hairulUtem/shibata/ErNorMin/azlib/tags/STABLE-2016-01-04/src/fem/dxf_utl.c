...
/* $Id: dxf_utl.c 842 2006-12-04 06:21:20Z sasaoka $ */
...
RC
output_dxf_3dface_lay (FILE *fp, char **layer, int num_layer,
                       ELEMENT_ARRAY elem, NODE_ARRAY node)
{
...
RC
output_dxf_3dface (FILE *fp, char *def_layer, int def_color,
                   ELEMENT_ARRAY elem, NODE_ARRAY node)
{
...
static RC
output_dxf_3dface_base (FILE *fp, char *def_layer, int def_color, char **layer,
                        int num_layer, ELEMENT_ARRAY elem, NODE_ARRAY node)
{
	int ii1,ii2;
	int index;
	double xyz[12];
...
RC
output_dxf_polyline (FILE *fp, ELEMENT_ARRAY element, NODE_ARRAY node)
{
...
RC
output_dxf_polyline_lay (FILE *fp, char *layer,
                         ELEMENT_ARRAY elem, NODE_ARRAY node)
{
	int ii1;
...
static void
write_dxf_3dface (FILE *fp, double *xyz, char *layer, int color)
{
	int ii1, ii2;
	int index;
...
RC
input_dxf (FILE *fp, ELEMENT_ARRAY *element, NODE_ARRAY *node)
{
	int ret_zero, dummy;
...
RC
input_dxf_3dface (FILE *fp, ELEMENT_ARRAY *element, NODE_ARRAY *node)
{
	int dummy;
...
RC
input_dxf_3dface_lay (FILE *fp, char ***layer, int *num_layer,
                      ELEMENT_ARRAY *element, NODE_ARRAY *node)
{
	...
	while(1){
		...
		if(layer != NULL){
			...
			if(l_index < 0) return(ALLOC_ERROR_RC);
...
static int
layer_index (int *numlayer, char ***layer, char *str)
{
	int ii1;
	char **lay_tmp;
	...
	(*numlayer)++;
	lay_tmp = (char **)mm_realloc((*layer), (*numlayer)*sizeof(char *));
	if(lay_tmp == NULL) return(-1);
	(*layer) = lay_tmp;

	(*layer)[(*numlayer) - 1] = (char *)mm_alloc(strlen(str)+1);
	if((*layer)[(*numlayer) - 1] == NULL) return(-1);
...
static RC
read_dxf_3dface (FILE *fp, char **layer, int *num_node,
                 double *xyz, int *color)
{
	int ret_zero;
	int ii1;
...
RC
input_dxf_polyline (FILE *fp, ELEMENT_ARRAY *element, NODE_ARRAY *node)
{
	int dummy;
...
RC
input_dxf_polyline_lay (FILE *fp, char ***layer, int *num_layer,
                        ELEMENT_ARRAY *element, NODE_ARRAY *node)
{
	int ret_zero;
	int ii1;
