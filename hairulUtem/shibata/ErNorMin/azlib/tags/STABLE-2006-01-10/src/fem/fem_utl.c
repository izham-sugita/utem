...
/* $Id: fem_utl.c 544 2005-11-04 14:21:07Z aoyama $ */
...
FUNC_FREE_ARRAY(free_geom_segment_array, GEOM_SEGMENT_ARRAY)
...
FUNC_COPY_ARRAY(copy_geom_segment_array, GEOM_SEGMENT_ARRAY, GEOM_SEGMENT)
...
FUNC_REALLOC_ARRAY(realloc_geom_segment_array, GEOM_SEGMENT_ARRAY,
                   GEOM_SEGMENT, init_geom_segment)
...
RC
allocate_geom_segment_array (int num, GEOM_SEGMENT_ARRAY *segment)
{
	int ii1;

	if(num > 0){
		segment->array = mm_alloc(num * sizeof(GEOM_SEGMENT));
		if(segment->array == NULL) return(ALLOC_ERROR_RC);
	}else{
		segment->array = NULL;
	}

	segment->size = 0;
	segment->alloc_size = num;
	segment->sort_flag = ARRAY_UNSORTED;

	for(ii1=0; ii1<num; ii1++){
		init_geom_segment( &((segment->array)[ii1]) );
	}
 ...
 * 使用する．同じ label が連続している場合はできる限り小さい indexを
 * 返却する
 */
...
RC node_location_matrix (ELEMENT *element,
                         NODE_ARRAY node, double **matrix)
{
	int ii1;
	...
	for(ii1=0; ii1<node.size; ii1++){
		...
		if(elem_lookup->array[ii1].alloc_size > 0){
			elem_lookup->array[ii1].element
			     = mm_alloc(sizeof(int)*(elem_lookup->array[ii1].alloc_size));
			if(elem_lookup->array[ii1].element == NULL) return(ALLOC_ERROR_RC);
		}
	}
