...
/* $Id: nst_f06_component.c 415 2005-07-15 06:04:24Z sasaoka $ */
...
static void idc2strain_3d(const IDC *idc, const IDC *idc2, const IDC *idc3,
                          int elem_id, int node_id, STRESS_STRAIN *strain);
static RC approach_f06strain(FILE *fp, ELEM_TYPE elem_type);
...
static RC read_f06ss(FILE *fp, int subcase, double load_step, int mode,
                     int f06_sw, STRAIN_ARRAY *strain,
                     const char *key, int f06_ss);



RC
nst_input_disp (FILE *fp, DISP_ARRAY *disp)
{
...
RC
nst_input_eigenvector_value (FILE *fp, DISP_ARRAY *evect, double value)
{
...
RC
nst_input_eigenvector_mode (FILE *fp, DISP_ARRAY *evect, int mode)
{
...
RC
nst_input_eigen_value (FILE *fp, int *array_size, double **value_array)
{
...
RC
read_f06stress (FILE *fp, int subcase, double load_step, int mode,
...
RC
read_f06strain (FILE *fp, int subcase, double load_step, int mode,
...
RC
read_f06eigen_value (FILE *fp, int subcase, int f06_sw, int *array_size,
                     double **value_array)
{
	int subc0;
	double loads0;
	...
	*array_size = 0;
	while(1){
		...
		tmp = (double *)mm_realloc(*value_array,
...
RC
read_f06disp (FILE *fp, int subcase, double load_step, int mode, int f06_sw,
...
static RC
seek_f06 (FILE *fp, const char *key, char *buf, int buf_size,
          int *subcase, double *load_step, int *mode)
{
...
static RC
check_f06key (const char *buf, const char *key)
{
	int len;
	int ii = 0;
...
static int
check_f06subcase_load (int f06_sw, int sub1, int sub2, double load1,
                       double load2, int mode1, int mode2)
{
	if(f06_sw & F06_SW_SUBCASE){
...
static RC
approach_f06eigen_value (FILE *fp)
{
	char buf[BUFFER_SIZE];
 ...
 */
static RC
read_f06ss (FILE *fp, int subcase, double load_step, int mode,
            int f06_sw, STRAIN_ARRAY *strain, const char *key, int f06_ss)
{
	int subc0;
	double loads0;
	int mode0;
	...
	RC_TRY( allocate_strain_array(INIT_SIZE, strain) );
	...
	RC_TRY( clean_strain_array(strain) );
...
static void
idc2strain_2d (const IDC *idc, int elem_id, int node_id, STRESS_STRAIN *strain)
{
...
static void
idc2strain_3d (const IDC *idc, const IDC *idc2, const IDC *idc3,
               int elem_id, int node_id, STRESS_STRAIN *strain)
{
	strain->element = elem_id;
	strain->node = node_id;
...
static RC
approach_f06strain (FILE *fp, ELEM_TYPE elem_type)
{
	char buf[BUFFER_SIZE];
...
#if 0
static void
set_tensor_stress (STRESS_ARRAY *stress)
{
	int ii1;
...
#if 0
static void
set_tensor_strain (STRAIN_ARRAY *strain)
{
	int ii1;
