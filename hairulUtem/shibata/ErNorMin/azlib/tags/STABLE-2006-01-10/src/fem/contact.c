...
/* $Id: contact.c 415 2005-07-15 06:04:24Z sasaoka $ */
...
#define REL_ERROR (DBL_EPSILON * 1.0e+10)
...
make_contact_array (ELEMENT_ARRAY element, NODE_ARRAY node,
                    ...
                    GEOM_SEGMENT_ARRAY (*segment)[3], CONTACT_ARRAY *contact)
{
	...
	/* elementの探査配列がなければ作る */
	if((*segment)[0].size == 0) 
		RC_TRY( make_geom_search_array(element, node, segment) );
	...
	for(ii1=0; ii1<target_node.size; ii1++){
		...
		{ /* target NODEを中心とする Bounding Box */
			min.x = target_node.array[ii1].p.x - range;
			min.y = target_node.array[ii1].p.y - range;
			min.z = target_node.array[ii1].p.z - range;
			max.x = target_node.array[ii1].p.x + range;
			max.y = target_node.array[ii1].p.y + range;
			max.z = target_node.array[ii1].p.z + range;
		}
...
/* mix-max範囲内にある全 elementにおいて point と各要素との */
/* 最短距離にある交点を計算し，CONTACT_TERM_SIZE 個検出する */
RC
make_contact_terminus (ELEMENT_ARRAY element, NODE_ARRAY node,
                       NODE point, VECT3D min, VECT3D max,
                       const GEOM_SEGMENT_ARRAY segment[], CONTACT *contact)
{
	int ii1, ii2;
	int size;
	int *label_array;
	...
	/* 前回の結果が入ってるかもしれないので開放 */
	RC_TRY( free_contact(contact) );

	/* 探査配列の中から，min < ? < max である elementのlabel配列を抜き出す */
	RC_TRY( geom_search_candidate(segment, min, max, &size, &label_array) );
	if(size <= 0){
	...
	for(ii1=0, ii2=0; ii1<size; ii1++){
		int index = search_element_label(element, label_array[ii1]);
		if(index < 0) return(SEEK_ERROR_RC);
		RC_TRY( contact_terminus_elem(element.array[index], node, point.p,
		                              &(contact->term[ii2])) );
		contact->size++;
		ii2++;
	}

	RC_TRY( mm_free(label_array) );
