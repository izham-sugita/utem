...
/* $Id: fem_solver_6dof.c 432 2005-08-24 03:00:31Z sasaoka $ */
...
static RC fill_row_nodes6(ELEM_LOOKUP elem_lookup,
...
static RC rigid_modify_nonzero6(NONZERO_MATRIX3 matrix,
               NODE_ARRAY node, RIGID_ELEMENT_ARRAY rigid,
...
static RC expand_rigid(NODE_ARRAY node, RIGID_ELEMENT_ARRAY src,
                       RIGID_ELEMENT_ARRAY *dest, int slave_table[]);
static RC expand_rigid_sub(NODE_ARRAY node, RIGID_ELEMENT rigid,
                           RIGID_ELEMENT_ARRAY *dest, int slave_table[]);
static RC cal_index_dof6(NODE_ARRAY node, int label, int src_dof,
                         int *index, int *dof);
static VECT3D trans_local2global(LOCAL_COORD coord, VECT3D local);
static void local_coord2s_matrix(LOCAL_COORD coord, double s_matrix[][3]);
static void local_coord2s_matrix_t(LOCAL_COORD coord, double s_matrix[][3]);
static RC rigid_recover_sub(int gdim, NODE_ARRAY node, int slave_flag[],
                            RIGID_ELEMENT_ARRAY rigid, double weight,
                            int index0, int dof0, int index1, int dof1,
                            double d_vect[], const int slave_table[]);
static RC rigid_recover(int gdim, NODE_ARRAY node,
                        RIGID_ELEMENT_ARRAY rigid, double d_vect[],
                        const int slave_table[]);
...
RC
fem_solve_iccg6 (NODE_ARRAY node, ELEMENT_ARRAY element,
                 RIGID_ELEMENT_ARRAY rigid,
                 MATERIAL_PROP_ARRAY material, PHYSICAL_PROP_ARRAY physical,
                 LOCAL_COORD_ARRAY coord, BC_ARRAY rest, BC_ARRAY force,
                 ...
                 STRAIN_ARRAY *strain, int sol_ss_flag)
{
	...
	RC_TRY( add_nodal_force6(node, coord, force, f_vect) );
	RC_TRY( add_temp_force(6, node, element, material, physical,
	                       temp, def_temp, f_vect) );
	RC_TRY( add_body_force(6, node, element, material, physical,
	                       def_body_force, f_vect) );
	RC_TRY( force_local_pre6(node, coord, f_vect) );

	/* 変位の計算 */
	RC_TRY( modify_nonzero6(node, coord, rest, rigid, matrix,
	                        &f_vect, 1, NULL) );
	RC_TRY( scale_iccg_nonzero3(matrix, f_vect, &d_vect, NULL) );
	RC_TRY( free_nonzero_matrix3(&matrix) );
	RC_TRY( fem_free_vector(&f_vect) );
	RC_TRY( rigid_recover6(node, rigid, d_vect) );
	RC_TRY( disp_local_recover6(node, coord, d_vect) );
	RC_TRY( vect2disp(6, node, d_vect, disp) );
...
/* ただし、剛性マトリックスは 3*3/ブロック */
RC
fem_solve_shell_iccg6 (NODE_ARRAY node, ELEMENT_ARRAY element,
                       RIGID_ELEMENT_ARRAY rigid,
                       ...
                       STRESS_ARRAY *moment, STRESS_ARRAY *shear_force,
                       STRAIN_ARRAY *curv, int sol_ss_flag)
{
	...
	for(ii1=0; ii1<element.size; ii1++){
		...
		RC_TRY( progress_printf(5, " [%6d/%6d]",
		                        element.array[ii1].label, element.size) );
		RC_TRY( make_shell_matrix(element.array[ii1], node, material,
...
**************/
	...
	/* ひずみ、応力の計算 */
	RC_TRY( stress_strain_shell(sol_ss_flag, distance, node, element,
	                            *disp, material, physical, stress, strain));
	/* 曲率、合応力の計算 */
	RC_TRY( resultant_stress_strain_shell(sol_ss_flag, node, element,
...
RC
allocate_nonzero6 (NODE_ARRAY node, ELEMENT_ARRAY element,
                   RIGID_ELEMENT_ARRAY rigid_elem, NONZERO_MATRIX3 *matrix)
{
	...
	/* 作業用配列 */
	RC_NULL_CHK( row_nodes1 = (int *)mm_alloc(2*node_size*sizeof(int)) );
	RC_NULL_CHK( row_nodes2 = (int *)mm_alloc(2*node_size*sizeof(int)) );
	RC_NULL_CHK( cache = (int(*)[2])mm_alloc(2*cache_size*sizeof(int)) );
	...
	for(ii1=0; ii1<elem_lookup.size; ii1++){
		...
		RC_NULL_CHK( matrix->row[index1].elem
		             = mm_alloc(row_size1*sizeof(NONZERO_ELEM3)) );
		for(ii2=0; ii2<row_size1; ii2++){
		...
		RC_NULL_CHK( matrix->row[index2].elem
		             = mm_alloc(row_size2*sizeof(NONZERO_ELEM3)) );
		for(ii2=0; ii2<row_size2; ii2++){
	...
	RC_TRY( mm_free(row_nodes1) );
	RC_TRY( mm_free(row_nodes2) );
	RC_TRY( mm_free(cache) );
...
static RC
fill_row_nodes6 (ELEM_LOOKUP elem_lookup,
                 NODE_ARRAY node, ELEMENT_ARRAY element,
                 int row_nodes1[], int *row_size1,
                 int row_nodes2[], int *row_size2,
                 int cache_size, int cache[][2])
{
	...
	*row_size2 = 1;
	...
	/* ソートしておく */
	sort_int_array(*row_size1, row_nodes1);
	sort_int_array(*row_size2, row_nodes2);
...
RC
impose_nonzero6 (NONZERO_MATRIX3 matrix,
...
static RC
rigid_modify_nonzero6 (NONZERO_MATRIX3 matrix, NODE_ARRAY node,
                       RIGID_ELEMENT_ARRAY rigid, int slave_flag[],
                       int master_flag[], double master_weight[],
                       int *master_count, double **f_vect, int vect_size,
                       double weight, int label0, int dof0, int label, int dof,
                       const int slave_table[])
...
static RC
expand_rigid (NODE_ARRAY node, RIGID_ELEMENT_ARRAY src,
              RIGID_ELEMENT_ARRAY *dest, int slave_table[])
{
	int ii1;
	int index, dof;
	...
	RC_TRY( allocate_rigid_element_array(src.size, dest) );
...
static RC
expand_rigid_sub (NODE_ARRAY node, RIGID_ELEMENT rigid,
                  RIGID_ELEMENT_ARRAY *dest, int slave_table[])
{
	int ii1, ii2;
	VECT3D p0, p;
	...
	for(ii1=1; ii1<rigid.node_num; ii1++){
		RC_NEG_CHK( n_index = search_node_label(node, rigid.node[0]) );
		p0 = node.array[n_index].p;

		RC_NEG_CHK( n_index = search_node_label(node, rigid.node[ii1]) );
		...
		if(dof_chk(rigid.dof[0], 1)){
			...
			RC_NULL_CHK( dest->array[dest->size].node
			                 = mm_alloc(4 * sizeof(int)) );
			RC_NULL_CHK( dest->array[dest->size].weight
			                 = mm_alloc(4 * sizeof(double)) );
		...
		for(ii2=3; ii2<6; ii2++){
			if(dof_chk(rigid.dof[0], 1 + ii2)){
				...
				RC_NULL_CHK( dest->array[dest->size].node
				                 = mm_alloc(2 * sizeof(int)) );
				RC_NULL_CHK( dest->array[dest->size].weight
				                 = mm_alloc(2 * sizeof(double)) );
				RC_NULL_CHK( dest->array[dest->size].dof
				                 = mm_alloc(2 * sizeof(int)) );
				dest->array[dest->size].node[0] = rigid.node[ii1];
				dest->array[dest->size].node[1] = rigid.node[0];
...
static VECT3D
trans_local2global (LOCAL_COORD coord, VECT3D local)
{
	double s_matrix[3][3];
	VECT3D ret;
...
static void
local_coord2s_matrix (LOCAL_COORD coord, double s_matrix[][3])
{
...
static void
local_coord2s_matrix_t (LOCAL_COORD coord, double s_matrix[][3])
{
	s_matrix[0][0] = coord.d_cos[0].x;
...
#if 0
RC
make_beam_mass_matrix (ELEMENT element, NODE_ARRAY node,
                       MATERIAL_PROP_ARRAY material,
                       PHYSICAL_PROP_ARRAY physical,
                       ELEM_MATRIX *elem_matrix)
{
	...
	RC_NEG_CHK( n_index = search_node_label(node, element.node[0]) );
	p1 = node.array[n_index].p;

	RC_NEG_CHK( n_index = search_node_label(node, element.node[1]) );
                       ...
                       ELEM_MATRIX *elem_matrix)
{
	...
	/* elem_matirx->index[] の確保と代入 */
	elem_matrix->index = mm_alloc(sizeof(int)*(elem_matrix->size));
	if(elem_matrix->index < 0) return(ALLOC_ERROR_RC);
	for(ii1=0; ii1<element.node_num; ii1++){
		RC_NEG_CHK( r_index = node_renum_index(node, element.node[ii1]) );
		for(ii2=0; ii2<(elem_matrix->dim); ii2++){
			elem_matrix->index[ii1*(elem_matrix->dim)+ii2]
	...
	len = abs_vect3d( pd = sub_vect3d(p2, p1) );
	pd = mul_scalar_vect3d(1.0/len, pd);
	vtmp = mul_scalar_vect3d(inner_product3d(v1, pd), pd);
	v1 = sub_vect3d(v1, vtmp);
	v1 = mul_scalar_vect3d(1.0/abs_vect3d(v1), v1);
	v2 = outer_product3d(pd, v1);
...
/* 要素剛性マトリックスの作成 */
RC
make_beam_matrix (ELEMENT element, NODE_ARRAY node,
                  MATERIAL_PROP_ARRAY material, PHYSICAL_PROP_ARRAY physical,
                  ELEM_MATRIX *elem_matrix)
{
	int ii1, ii2, ii3, ii4;
...
/* 節点バネ要素の処理 */
RC
modify_spring6 (NODE_ARRAY node, RIGID_ELEMENT_ARRAY src_rigid,
...
RC
modify_node_matrix6 (NODE_ARRAY node, NODE_MATRIX_ARRAY nmat,
...
/* fact: 係数         */
RC
modify_mass6 (NODE_ARRAY node, RIGID_ELEMENT_ARRAY rigid,
              double fact, NONZERO_MATRIX3 matrix)
{
	int ii1;
	int index;
...
RC
modify_nonzero6 (NODE_ARRAY node, LOCAL_COORD_ARRAY coord,
                 ...
                 double **f_vect, int vect_size, int active_dof_flag[])
{
	...
	for(ii1=0; ii1<node.size; ii1++){
		...
		if( (node.source == FEM_NASTRAN)
		  &&(node.array[ii1].i_info[1] > 0) ){
			int coord_index
			     = search_local_coord_label(coord,
	...
	if(src_rigid.size > 0){
		RC_NULL_CHK( slave_table = (int *)mm_alloc(6*node.size*sizeof(int)) );
		RC_NULL_CHK( master_flag = (int *)mm_alloc(6*node.size*sizeof(int)) );
		RC_NULL_CHK( master_weight
		             = (double *)mm_alloc(6*node.size*sizeof(double)) );
		RC_TRY( expand_rigid(node, src_rigid, &rigid, slave_table) );
		RC_NULL_CHK( slave_flag  = (int *)mm_alloc(rigid.size*sizeof(int)) );
	}else{
		rigid.size = 0;
	}
	...
	if(rigid.size > 0){
		RC_TRY( mm_free(slave_table) );
		RC_TRY( mm_free(slave_flag) );
		RC_TRY( mm_free(master_flag) );
		RC_TRY( mm_free(master_weight) );
		RC_TRY( free_rigid_element_array(&rigid) );
	}
...
static RC
cal_index_dof6 (NODE_ARRAY node, int label, int src_dof, int *index, int *dof)
{
...
static RC
add_master_dof (NONZERO_MATRIX3 matrix, int master_count, int master_flag[],
                double master_weight[], int index0, int dof0)
{
	int count = 0;
	int ii1, ii2, ii3;
...
static void
init_flags (int flags[], int size, int i)
{
	int ii1;
...
RC
rigid_recover6 (NODE_ARRAY node, RIGID_ELEMENT_ARRAY src_rigid,
                double d_vect[])
{
	RIGID_ELEMENT_ARRAY rigid;
	int *slave_table;
	...
	RC_NULL_CHK( slave_table = (int *)mm_alloc(6*node.size*sizeof(int)) );
	RC_TRY( expand_rigid(node, src_rigid, &rigid, slave_table) );
	RC_TRY( rigid_recover(6, node, rigid, d_vect, slave_table) );
	RC_TRY( free_rigid_element_array(&rigid) );
	RC_TRY( mm_free(slave_table) );
...
/* スレーブ自由度にマスター自由度を反映させる */
static RC
rigid_recover (int gdim, NODE_ARRAY node, RIGID_ELEMENT_ARRAY rigid,
               double d_vect[], const int slave_table[])
{
	int ii1, ii2, ii3;
	int index0, index1;
...
static RC
rigid_recover_sub (int gdim, NODE_ARRAY node, int slave_flag[],
                   RIGID_ELEMENT_ARRAY rigid, double weight,
                   int index0, int dof0, int index1, int dof1,
                   double d_vect[], const int slave_table[])
{
...
/* 荷重ベクトルに節点荷重を加算 */
RC
add_nodal_force6 (NODE_ARRAY node, LOCAL_COORD_ARRAY coord,
	...
	for(ii1=0; ii1<force.size; ii1++){
		...
		if( (force.source == FEM_NASTRAN)
		  &&(force.array[ii1].i_info[0] > 0) ){
			double s_matrix[3][3];
			int coord_index = search_local_coord_label(coord,
...
/* 荷重ベクトルに圧力/表面荷重を加算 */
RC
add_pressure_force6 (NODE_ARRAY node, ELEMENT_ARRAY element,
                     LOCAL_COORD_ARRAY coord,
                     ELEM_BC_ARRAY press, double *f_vect)
{
	...
	for(ii1=0; ii1<press.size; ii1++){
		...
		RC_TRY( make_face_table(element.array[elem_index].type, &f_table) );
		...
		if( (press.array[ii1].type == BC_PRESS)
		  ...
		  ||(press.array[ii1].type == BC_PRESS_D2) ){
			...
			VECT3D gdir = press.array[ii1].dir;

			if( (press.source == FEM_NASTRAN)
			  &&(press.array[ii1].i_info[0] > 0) ){
				int index = search_local_coord_label(coord,
...
/* 荷重ベクトルを節点の座標系に合わせる */
RC
force_local_pre6 (NODE_ARRAY node, LOCAL_COORD_ARRAY coord, double f_vect[])
{
	int ii1;
	int coord_index;
	int index;
	...
	for(ii1=0; ii1<node.size; ii1++){
		...
		if( (node.source == FEM_NASTRAN)&&(node.array[ii1].i_info[1] > 0) ){
			coord_index = search_local_coord_label(coord,
			                                    node.array[ii1].i_info[1]);
...
/* 変位ベクトルを全体座標系に戻す */
RC
disp_local_recover6 (NODE_ARRAY node, LOCAL_COORD_ARRAY coord,double d_vect[])
{
	int ii1;
	int coord_index;
	int index;
...
RC
nonzero2skyline6 (NONZERO_MATRIX3 nonzero, SKYLINE_MATRIX *skyline,
