...
/* $Id: fem_struct.h 415 2005-07-15 06:04:24Z sasaoka $ */
...
typedef struct {
	...
	VECT3DR G_vect;      /* Shear mdulus(yz,zx,xy) */
...
typedef struct {
	...
	VECT3DR v;       /* stress(or strain) vector x,y,z,yz,zx,xy */
...
void print_geom_segment(FILE *fp, GEOM_SEGMENT segment);
...
void print_geom_segment_array(FILE *fp, GEOM_SEGMENT_ARRAY segment);
...
RC free_geom_segment_array(GEOM_SEGMENT_ARRAY *segment);
...
RC copy_geom_segment_array(GEOM_SEGMENT_ARRAY src, GEOM_SEGMENT_ARRAY *dest);
...
RC realloc_geom_segment_array(GEOM_SEGMENT_ARRAY *segment);
...
RC allocate_geom_segment_array(int num, GEOM_SEGMENT_ARRAY *segment);
...
/* contact.c */
RC make_contact_array(ELEMENT_ARRAY element, NODE_ARRAY node,
...
RC make_contact_terminus(ELEMENT_ARRAY element, NODE_ARRAY node,
                         ...
                         const GEOM_SEGMENT_ARRAY segment[],
                         CONTACT *contact);
...
RC make_geom_search_array(ELEMENT_ARRAY elem, NODE_ARRAY node,
                          GEOM_SEGMENT_ARRAY (*segment)[3]);
RC geom_search_candidate(const GEOM_SEGMENT_ARRAY segment[],
