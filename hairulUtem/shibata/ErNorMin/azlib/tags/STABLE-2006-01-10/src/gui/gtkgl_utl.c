...
/* $Id: gtkgl_utl.c 450 2005-09-12 05:07:22Z sasaoka $ */
...
draw_gradation_backgroud (void)
{
	...
	for(ii1=0; ii1<64; ii1++){
		...
		color[0] = (unsigned char)255*(gtku_gl->bg_color0[0]+ii1*color_grad[0]);
		color[1] = (unsigned char)255*(gtku_gl->bg_color0[1]+ii1*color_grad[1]);
		color[2] = (unsigned char)255*(gtku_gl->bg_color0[2]+ii1*color_grad[2]);
	...
	glLoadIdentity();
	glRasterPos2i(0, 0);
	glPixelZoom(gtku_gl->view_size.x*2/64, gtku_gl->view_size.y*2/64); 
