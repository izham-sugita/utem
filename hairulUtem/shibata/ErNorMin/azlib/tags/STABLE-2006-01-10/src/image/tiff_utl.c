...
/* $Id: tiff_utl.c 432 2005-08-24 03:00:31Z sasaoka $ */
...
RC
read_tiff_header (FILE *fp, GRAPHIC *gr, ENDIAN *endian)
{
	RC rc;
	int ii1;
	int itmp;
	...
	/* IFD Position に移動 */ 
	if(fseek(fp, ul, SEEK_SET) != 0) return(SEEK_ERROR_RC);

	/* IFD0 タグ数 */
	RC_TRY( read_2bytes(fp, *endian, &ui) );
	if(ui < 1) return(READ_ERROR_RC);
	num_tag = ui;
			...
			case 258: /* 色ビット数[bit/dot] */
				...
				if(num_data == 1){
					if((ul==8)||(ul==1)){
						...
						RC_TRY( error_printf(7003, (int)ul) );
						return(READ_ERROR_RC);
					}
				}else if(num_data > 1){
...
RC
read_tiff_image (FILE *fp, GRAPHIC *gr, ENDIAN endian)
{
	int ii1, ii2, ii3;
	int cl, cl1;
	GRAPHIC_RGB palette[256];

	if(gr->i_info[1] < 0) return(READ_ERROR_RC);
	...
	if((gr->i_info[0] == 24)&&(gr->i_info[3] == 2)){
		...
		RC_TRY( error_printf(7003, gr->i_info[0]) );
		RC_TRY( error_printf(7403, gr->i_info[3]) );
		return(READ_ERROR_RC);
	}
...
RC static
read_tiff_palette (FILE *fp, GRAPHIC *gr, ENDIAN endian, GRAPHIC_RGB *palette)
{
	int ii1;
	unsigned int ui;


	if(gr->i_info[2] < 0) return(READ_ERROR_RC);
