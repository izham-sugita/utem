...
/* $Id: avs_field_utl.c 432 2005-08-24 03:00:31Z sasaoka $ */
...
RC
read_avs_field (FILE *fp, GRAPHIC *gr)
{
...
RC
read_avs_field_heder (FILE *fp, GRAPHIC *gr)
{
	long int current;
...
static RC
cast_value (GRAPHIC *gr, const char *str1, const char *str2)
{
	IDC idc[10];
...
/* サンプルデータが無い，もしくは必要性が無いために保留
		...
		char *label = NULL;
		if(label == (char *)NULL){
			RC_NULL_CHK( label = (char *)mm_alloc(itmp+1) );
			strncpy(label, str2, itmp);
			label[itmp+1] = '\0';
		}else{
			label = (char *)mm_realloc(label, strlen(label)+itmp+1);
			if(label == NULL) return(NORMAL_RC);
			label[strlen(label)-1] = ' ';
			strncat(label, str2, itmp);
		}
	}else if(strncmp(str1, "unit", 4) == 0){
		char *unit ;
		RC_NULL_CHK( unit = (char *)mm_alloc(STRING_SIZE) );
		strncpy(unit, str2, STRING_SIZE-1);
		unit[STRING_SIZE] = '\0';
	}else if(strncmp(str1, "min_val", 7) == 0){
		char *min_val;
		RC_NULL_CHK( min_val = (char *)mm_alloc(STRING_SIZE) );
		strncpy(min_val, str2, STRING_SIZE-1);
		min_val[STRING_SIZE] = '\0';
	}else if(strncmp(str1, "max_val", 7) == 0){
		char *max_val;
		RC_NULL_CHK( max_val = (char *)mm_alloc(STRING_SIZE) );
...
RC
read_avs_field_data (FILE *fp, GRAPHIC *gr)
{
...
RC
read_avs_field_data_value (FILE *fp, GRAPHIC *gr)
{
	unsigned int ui;
	unsigned long ul;
...
RC
read_avs_field_data_coord (FILE *fp, GRAPHIC *gr)
{
	if(gr->i_info[2] <= 0) return(ARG_ERROR_RC);
...
RC
write_avs_field_header (FILE *fp, GRAPHIC gr)
{
	fprintf(fp, "# AVS field file \n");
	fprintf(fp, "ndim = %d\n", gr.i_info[1]);

	if(gr.i_info[1] == 0) return(ARG_ERROR_RC);
...
RC
write_avs_field_data (FILE *fp, GRAPHIC gr)
{
	unsigned int ui;
	unsigned long ul;
