...
/* $Id: memory_manager.c 428 2005-08-23 07:48:57Z sasaoka $ */
...
mm_terminate (void)
{
	if( (mm.ptr == NULL)&&(mm.list == NULL)
	  &&(mm.size == 0)&&(mm.max_usage == 0) ){
		/* 確保されていない */
		return(NORMAL_RC);
	}
...
mm_alloc_sub (size_t size, size_t max_seek_count)
{
	...
	if( (mm.ptr == NULL)&&(mm.list == NULL)
	  &&(mm.size == 0)&&(mm.max_usage == 0) ){
		/* 確保されていない，とりあえず 16MB 確保 */
		if(mm_init(16*10241024) != NORMAL_RC) return(NULL);
	}
...
mm_chk_list (FILE *fp)
{
	...
	if(fp != NULL){
		fprintf(fp, "ptr = %p, list = %p, size = %lu, MM_LIST_SIZE = %lu\n",
		        mm.ptr, mm.list, (unsigned long)mm.size,
		        (unsigned long)MM_LIST_SIZE);
	}
	...
	while(list_ptr != NULL){
		...
		if(fp != NULL){
			...
			fprintf(fp, " %10lu ", (unsigned long)list_ptr->size);
			fprintf(fp, " %10lu ", (unsigned long)list_ptr->chk_sum);
...
is_error_list (MM_LIST *list_ptr)
{
	...
	if(list_ptr->chk_sum != cal_chk_sum(list_ptr)) return(1);
	if( (list_ptr->alloc_flag != 0)
	  &&(list_ptr->alloc_flag != 1)
	  &&(list_ptr->alloc_flag != 2) ){
		return(1);
