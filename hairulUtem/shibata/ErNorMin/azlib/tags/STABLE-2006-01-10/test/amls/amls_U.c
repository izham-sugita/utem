            ...
            MATRIX_DB_INFO *mdbi_U)
{
	...
	for(ii1=0; ii1<block_num; ii1++){
		...
		for(ii2=0; ii2<block_size3; ii2++){
			RC_TRY( get_row_col_matrix_db(mdbi_K,ii2+diag_pos3, diag_pos3,
	...
	RC_TRY( allocate2D(total_size, total_size, &ZtUtMUZ) );
                    ...
                    MATRIX_DB_INFO *mdbi_U)
{
	...
	for(ii1=1; ii1<block_num; ii1++){
		...
		Zt_current_pos += evects_size_array[ii1-1];
		Zt_next_pos = Zt_current_pos + evects_size_array[ii1]; 
		...
		for(ii2=0; ii2<ii1; ii2++){
			...
			for(ii3=Zt_current_pos; ii3<Zt_next_pos; ii3++){
                          ...
                          double **block_U, int **zero_flag)
{
	...
	for(ii1=0; ii1<col_size; ii1++){
		...
		index_ii1_3 = (col_array[ii1] - elim_pos) * 3;
