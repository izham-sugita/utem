...
static RC UtMU_chk(int row_size, int *block_size_array, MATRIX_DB_INFO *mdbi_U,
...
DEBUG_PRINT;
	...
	DEBUG_PRINT;
//	RC_TRY( UtMU_chk(row_size, block_size_array, mdbi_U,  mdbi_M) );
#if 0
{
	double **K;
	double **M;
...
static RC
UtMU_chk(int row_size, int *block_size_array, MATRIX_DB_INFO *mdbi_U,
         MATRIX_DB_INFO *mdbi_M)
{
	...
	RC_TRY( allocate1D33(row_size, &array2) );
	RC_TRY( allocate1I(row_size, &col_array2) );
	RC_TRY( allocate1D33(row_size, &arrayM) );
	RC_TRY( allocate1I(row_size, &col_arrayM) );
	...
	for(ii1=0; ii1<row_size; ii1++){
		...
		RC_TRY( get_row_matrix_db(mdbi_M, ii1,&col_sizeM,arrayM,col_arrayM) );
		for(ii2=0; ii2<col_sizeM; ii2++){
			RC_TRY(get_row_matrix_db(mdbi_U,ii2,&col_size2,array2,col_array2));
			for(ii3=0; ii3<col_size; ii3++){
				for(ii4=0; ii4<col_size2; ii4++){
					for(ii5=0; ii5<3; ii5++){
						for(ii6=0; ii6<3; ii6++){
							for(ii7=0; ii7<3; ii7++){
								for(ii8=0; ii8<3; ii8++){
									UtMU[col_array[ii3]*3+ii7]
									    [col_array2[ii4]*3+ii8]
									+= array[ii3][ii5][ii8]*
									   arrayM[ii2][ii5][ii6]*
									   array2[ii4][ii2][ii8];
								}
	...
	for(ii1=block_size_array[0]*3; ii1<total_size; ii1++){
		RC_TRY( log_printf(5, "utmu[%d][%d] = %e\n") );
	}
#endif
	...
	RC_TRY( free1D33(row_size, &array2) );
	RC_TRY( free1I(row_size, &col_array2) );
	RC_TRY( free1D33(row_size, &arrayM) );
	RC_TRY( free1I(row_size, &col_arrayM) );
		...
		for(ii2=0; ii2<ii1; ii2++){
			...
			for(ii3=0; ii3<block_size_array[ii1]; ii3++){
				...
				for(ii4=0; ii4<col_size; ii4++){
					...
					for(ii5=0; ii5<block_size_array[ii2]; ii5++){
						for(ii6=0; ii6<3; ii6++){
							for(ii7=0; ii7<3; ii7++){
								for(ii8=0; ii8<3; ii8++){
									block_U[ii5*3+ii8][ii3*3+ii6]
									  -= d_block_inv[ii5*3+ii8]
									     [(col_array[ii4]-elim_pos)*3+ii7]
									   * array[ii4][ii6][ii7];
								}
							}
						}
						...
						for(ii6=0; ii6<3; ii6++){
							for(ii7=0; ii7<3; ii7++){
								for(ii8=0; ii8<3; ii8++){
									...
									+= array[ii4][ii6][ii7]
									 * block_U[(col_array[ii4]-elim_pos)*3+ii7]
									          [(ii5-current_pos)*3+ii8];
								}
							}
						}
					}
				...
				for(ii4=0; ii4<next_pos-current_pos; ii4++){
					if(ii3 >= ii4+current_pos){
						if( nonzero_chk33(temp_array[ii4]) != 0 ){
							RC_TRY( add_v_matrix_db(ii3, ii4+current_pos,
...
#if 0
				RC_TRY( get_row_matrix_db(mdbi_U, ii3, &col_size, array,
			 	                          col_array) );
#endif
#if 1
				RC_TRY( get_row_col_matrix_db(mdbi_U, ii3, elim_pos,
				                              &col_size, array, col_array) );
#endif
				for(ii4=0; ii4<col_size; ii4++){
//					if(col_array[ii4] < elim_pos) continue;
					if(col_array[ii4] >= next_elim_pos) break;
					for(ii5=current_pos; ii5<next_pos; ii5++){
						for(ii6=0; ii6<3; ii6++){
							for(ii7=0; ii7<3; ii7++){
