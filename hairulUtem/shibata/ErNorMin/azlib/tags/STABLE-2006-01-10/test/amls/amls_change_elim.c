...
int main(int argc, char **argv)
{
	...
	MATERIAL_PROP_ARRAY material;
	PHYSICAL_PROP_ARRAY physical;
	int block_num = 30; /* row sizeの分割数(ブロック数) */
	int cache_M = 64;
	int cache_K = 128;
	int cache_U = 128;
                     ...
                     MATRIX_DB_INFO *mdbi_K, MATRIX_DB_INFO *mdbi_U)
{
	...
	for(ii1=0; ii1<block_num-1; ii1++){
		...
		for(ii2=ii1+1; ii2<block_num; ii2++){
			...
			int block_U_col_size = block_size_array[ii2] * 3;
			int chk_flag;
			...
			RC_TRY( allocate2D(block_U_row_size, block_U_col_size, &block_U) );
			for(ii3=current_pos; ii3<next_pos; ii3++){
				int block_U_ii3_3 = (ii3 - current_pos) * 3;
				...
				RC_TRY( s_cholesky_subst_sp3(skyline.dof, skyline.index1,
				                             ...
				                             temp_vect[2], 1) );
				for(ii4=0; ii4<3; ii4++){
					for(ii5=0; ii5<block_U_row_size; ii5++){
						block_U[ii5][block_U_ii3_3+ii4] = -temp_vect[ii4][ii5];
			...
			RC_TRY( free2I(next_elim_pos-elim_pos, next_pos-current_pos,
			               &zero_flag) );
			RC_TRY( free2D(block_U_row_size, block_U_col_size, &block_U) );
			current_pos += block_size_array[ii2];
		}
		elim_pos += block_size_array[ii1];
		if(decomp_flag == 0){
          ...
          double array[][3][3], double **block_U, int **zero_flag)
{
	...
	for(ii1=current_pos; ii1<row_size; ii1++){
		...
		for(ii2=0; ii2<col_size; ii2++){
			...
			U_ii2_3 = (col_array[ii2] - elim_pos) * 3;
			U_ii2 = col_array[ii2] - elim_pos;
			for(ii3=current_pos; ii3<next_pos; ii3++){
				int U_ii3_3;
				int U_ii3 = ii3 - current_pos;

				if(zero_flag[U_ii2][U_ii3]){
					continue;
				}

				U_ii3_3 = (ii3 - current_pos) * 3;
				for(ii4=0; ii4<3; ii4++){
					for(ii5=0; ii5<3; ii5++){
						for(ii6=0; ii6<3; ii6++){
                  ...
                  double array[][3][3], double **block_U, int **zero_flag)
{
	...
	for(ii1=elim_pos; ii1<next_elim_pos; ii1++){
		for(ii2=0; ii2<temp_col_size; ii2++){
			if(zero_flag[ii1-elim_pos][ii2]) continue;
			for(ii3=0; ii3<3; ii3++){
				for(ii4=0; ii4<3; ii4++){
...
{
	int ii1, ii2, ii3, ii4;
	int block_U_row3 = next_elim_pos - elim_pos;
	int block_U_col3 = next_pos - current_pos;


	for(ii1=0; ii1<block_U_row3; ii1++){
		for(ii2=0; ii2<block_U_col3; ii2++){
			int flag = 1;
			for(ii3=0; ii3<3; ii3++){
				for(ii4=0; ii4<3; ii4++){
