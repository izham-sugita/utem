...
static RC make_KM_matrix4full_mat(NODE_ARRAY node, ELEMENT_ARRAY element,
                        ...
                        double **K, double **M);
static RC result4full_chk (int row_size, int *block_size, double **K,
...
static RC make_U_matrix_dbi (MATRIX_DB_INFO *mdbi_U, int current_pos,
                             int next_pos, int elim_pos, int next_elim_pos,
                             int *col_array, double array[][3][3],
                             double **block_U, int **zero_flag);
static RC UtKU_matrix_dbi (MATRIX_DB_INFO *mdbi_K, int row_size,
                           int current_pos, int next_pos, int elim_pos,
                           int next_elim_pos,
                           int *col_array, double array[][3][3],
                           double **block_U, int **zero_flag);
static RC mul_U_matrix_dbi_block_U(int col_size, int current_pos, int next_pos,
                                   int elim_pos, int next_elim_pos,
...
#if 0
{
	...
	RC_TRY( result4full_chk(row_size, block_size_array, K, M, mdbi_U) );
...
static RC
result4full_chk (int row_size, int *block_size, double **K, double **M,
                 MATRIX_DB_INFO *mdbi_U)
{
	int ii1, ii2, ii3, ii4;
...
//	RC_TRY( eigen_jacobi_trans(total_size, UtMU, evals, evects) );
#if 1
	for(ii1=0; ii1<10; ii1++){
		RC_TRY( log_printf(5, "full_evals[%d] = % e\n",ii1,evals[ii1]) );
	}
#endif
            ...
            MATRIX_DB_INFO *mdbi_U)
{
	...
	/* Make Z */
	vect_size = 0;
	for(ii1=0; ii1<block_num; ii1++) vect_size += evects_size_array[ii1] * 3;
	...
	for(ii1=0; ii1<block_num; ii1++){
		...
		if(ii1<block_num){
#endif
			for(ii2=old_diag_pos3; ii2<diag_pos3; ii2++){
				double v[3][3];

				RC_TRY( get_v_matrix_db(ii2, ii2, v, &found_flag, mdbi_M) );
				if(!found_flag) return(UNKNOWN_ERROR_RC);
				for(ii3=0; ii3<3; ii3++){
					block_M[(ii2-old_diag_pos3)*3+ii3]
					       [(ii2-old_diag_pos3)*3+ii3] = v[ii3][ii3];
				}
			}
		}else{
...
//				RC_TRY(get_row_matrix_db(mdbi_U,ii2,&col_size,array,col_array));
				RC_TRY(get_row_col_matrix_db(mdbi_U, ii2, old_diag_pos3,
				                             &col_size, array, col_array));

				for(ii3=0; ii3<col_size; ii3++){
//					if(col_array[ii3] < old_diag_pos3) continue;
					if(col_array[ii3] >= diag_pos3) break;;
					for(ii4=0; ii4<=ii3; ii4++){
//						if(col_array[ii4] < old_diag_pos3) continue;
//						if(col_array[ii4] >= diag_pos3) break;;
						for(ii5=0; ii5<3; ii5++){
							for(ii6=0; ii6<3; ii6++){
								for(ii7=0; ii7<3; ii7++){
									block_M
									[(col_array[ii3]-old_diag_pos3)*3+ii6]
									[(col_array[ii4]-old_diag_pos3)*3+ii7]
		...
		old_diag_pos3 += block_size_array[ii1];
	...
	RC_TRY( cal_eval_evect(vect_size, ZtKZ, M, evals, evects) );
	for(ii1=0; ii1<10; ii1++){
                     ...
                     MATRIX_DB_INFO *mdbi_K, MATRIX_DB_INFO *mdbi_U)
{
	...
	for(ii1=1; ii1<block_num; ii1++){
		...
		for(ii2=0; ii2<ii1; ii2++){
			...
			/* ------------------ UtKU ------------------ */
			RC_TRY( UtKU_matrix_dbi(mdbi_K, row_size, current_pos, next_pos,
			                        elim_pos, next_elim_pos, col_array, array,
			                        block_U, zero_flag) );
			/* ------------------ U = U * block_U  ----------------- */
			RC_TRY( make_U_matrix_dbi (mdbi_U, current_pos, next_pos, elim_pos,
...
static RC
UtKU_matrix_dbi (MATRIX_DB_INFO *mdbi_K, int row_size, int current_pos,
...
static RC
make_U_matrix_dbi (MATRIX_DB_INFO *mdbi_U, int current_pos, int next_pos,
                   ...
                   double array[][3][3], double **block_U, int **zero_flag)
{
	...
	/* Check nonzero for U (0 -> elim_pos)*/
	total_count = 0;
	zero_count = 0;
	for(ii1=0; ii1<elim_pos; ii1++){
		RC_TRY( get_row_col_matrix_db(mdbi_U, ii1, elim_pos,
		                              &col_size, array, col_array) );
		for(ii2=0; ii2<col_size; ii2++){
	...
	RC_TRY( log_printf(5,
	"[0 -> elim_pos]: total_count = %d, zero_count = %d\n",
...
#endif
#if 1
		RC_TRY(mul_U_matrix_dbi_block_U(col_size, current_pos, next_pos,
		                                elim_pos, next_elim_pos, temp_array,
		                                array, col_array, block_U, zero_flag));
#endif
		/* 書き込み */
		for(ii2=0; ii2<next_pos-current_pos; ii2++){
			if( nonzero_chk33(temp_array[ii2]) != 0 ){
...
static RC
mul_U_matrix_dbi_block_U (int col_size, int current_pos, int next_pos,
                          int elim_pos, int next_elim_pos,
                          double temp_array[][3][3],
                          double array[][3][3], int col_array[],
                          double **block_U, int **zero_flag)
{
	...
	for(ii1=0; ii1<col_size; ii1++){
		if(col_array[ii1] >= next_elim_pos) break;
		for(ii2=current_pos; ii2<next_pos; ii2++){
			if(zero_flag[col_array[ii1]-elim_pos][ii2-current_pos]) continue;
			for(ii3=0; ii3<3; ii3++){
				for(ii4=0; ii4<3; ii4++){
					for(ii5=0; ii5<3; ii5++){
						temp_array[ii2-current_pos][ii3][ii5]
						+= array[ii1][ii3][ii4]
						 * block_U[(col_array[ii1]-elim_pos)*3+ii4]
						          [(ii2-current_pos)*3+ii5];
					}
				}
			}
		}
