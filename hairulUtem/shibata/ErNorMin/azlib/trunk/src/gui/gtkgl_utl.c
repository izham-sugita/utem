...
/* $Id: gtkgl_utl.c 1066 2016-05-19 09:53:33Z hayashi $ */
...
static gchar font_name[] = "Serif 16";  /* Fix!! 決め打ち */
...
/* gtku_gl の描画モードが mode であれば TRUE を返す．
 */
static gboolean
...
/* "realize" に対するシグナルハンドラ．
 * 通常は一度しか呼び出されない．
 * OpenGLの初期化が行われるが，この関数内ではモデルの大きさや中心の座標などが
 * 計算されるだけである．
 */
gboolean
...
/* "motion_notify_event" に対するシグナルハンドラ．
 * 描画領域内でマウスカーソルが動いている間に呼び出される．
 */
gboolean
...
/* "button_press_event" に対するシグナルハンドラ．
 * 
 * event_button:
 *   1: 左ボタン
 *   2: 中ボタン
...
/* "button_press_release" に対するシグナルハンドラ．
 * 
 * event_button:
 *   1: 左ボタン
 *   2: 中ボタン
