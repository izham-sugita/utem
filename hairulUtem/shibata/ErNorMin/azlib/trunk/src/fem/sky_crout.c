...
/* $Id: sky_crout.c 842 2006-12-04 06:21:20Z sasaoka $ */
 ...
 */
RC
s_crout_alloc (long n, const long *index1, long **index2,
...
/* 配列の解放 */
RC
s_crout_free (long *index2, double *array)
{
...
double
s_crout_get (long n, const long *index1, const long *index2,
...
double *
s_crout_ptr (long n, const long *index1, const long *index2,
...
/* 配列の内容を表示 (debug用) */
void
s_crout_print (FILE *stream, long n, const long *index1,
               const long *index2, const double *array)
{
	...
	for(ii1=0L;ii1<n;ii1++){
		...
		for(ii2=0L;ii2<n;ii2++){
			fprintf(stream, "%10.3f ",
			        s_crout_get(n,index1,index2,array,ii1,ii2,&rc));
		}
		fprintf(stream, "\n");
	}
}
...
RC
s_crout_mod (long n, long *index1, const long *index2,
...
/* クラウト法による分解 */
RC
s_crout_decomp (long n, const long *index1, const long *index2,
...
/* 前進代入、後退代入 */
RC
s_crout_subst (long n, const long *index1, const long *index2,
