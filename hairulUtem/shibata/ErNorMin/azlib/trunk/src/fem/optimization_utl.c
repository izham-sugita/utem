...
/* $Id: optimization_utl.c 1075 2016-10-31 09:48:59Z hayashi $ */
...
static RC make_matrix(NODE_ARRAY node, ELEMENT_ARRAY element,
                      MATERIAL_PROP_ARRAY material,
                      PHYSICAL_PROP_ARRAY physical,
                      NONZERO_MATRIX3 *nonzero, SKYLINE_MATRIX *skyline);
static RC restdisp_work(BC_ARRAY rest, REACT_ARRAY react, double *compliance);


/* H1勾配法で形状変動を求める．
 * 勾配（感度）tractions に対応する変動量 disps が得られる．
 * 領域写像の変位を「速度場」と呼び，それを求めることを「速度場解析」と
 * 呼んでいることから，このような関数名になっている．
 *
 * 先生の本では，右辺を勾配の -1 倍としているが，この関数では -1 倍していない．
 * その代わりに，後ほど求める荷重係数を -1 倍する．
 *
 * solver_flag でソルバーの種類を選ぶ．
 * 0: 疎行列LU分解法
 * 1: ICCG法
 */
RC
velocity_analysis (NODE_ARRAY node, ELEMENT_ARRAY element,
                   MATERIAL_PROP_ARRAY material,
                   PHYSICAL_PROP_ARRAY physical,
                   ...
                   int solver_flag)
{
	...
	/* 剛性マトリックス作成 */
	if(solver_flag == 0){
		RC_TRY(make_matrix(node, element, material, physical, NULL, &skyline));
	}else if(solver_flag == 1){
		RC_TRY(make_matrix(node, element, material, physical, &nonzero, NULL));
	}else{
	...
	/* 拘束条件組み込み */
	if(solver_flag == 0){
		RC_TRY(modify_g_matrix_n(node, shape_rest, skyline, f_vect, num_case));
	}else{ /* solver_flag == 1 */
		RC_TRY(modify_nonzero3_n(node, shape_rest, nonzero, f_vect, num_case));
	}

	/* スカイラインの場合，LU 分解 */
	if(solver_flag == 0) RC_TRY( decomp_g_matrix(skyline) );

	/* 変位の計算 */
	for(ii1=0; ii1<num_case; ii1++){
		if(solver_flag == 0){
			RC_TRY( subst_g_matrix(node, skyline, f_vect[ii1], &(disps[ii1])));
		}else{ /* solver_flag == 1 */
			double *d_vect;
			RC_TRY( iccg_nonzero3(nonzero, f_vect[ii1], &d_vect, NULL) );
			RC_TRY( vect2disp(3, node, d_vect, &(disps[ii1])) );
...
static RC
make_matrix (NODE_ARRAY node, ELEMENT_ARRAY element,
             ...
             NONZERO_MATRIX3 *nonzero, SKYLINE_MATRIX *skyline)
{
	...
	if(nonzero != NULL){
		RC_TRY( allocate_nonzero3(node, element, nonzero) );
		for(ii1=0; ii1<element.size; ii1++){
			...
			RC_TRY( impose_nonzero3(*nonzero, elem_matrix, 1.0) );
			RC_TRY( free_elem_matrix(&elem_matrix) );
		}
	}
	if(skyline != NULL){
		RC_TRY( make_g_matrix(node, element, material, physical, skyline) );
	}
...
RC
add_shape_rest (ELEMENT_ARRAY element, MATERIAL_PROP_ARRAY material,
                PHYSICAL_PROP_ARRAY physical, BC_ARRAY *shape_rest)
{
	...
	for(ii1=0; ii1<element.size; ii1++){
		RC_NEG_CHK( mat_index = search_material_prop_element(material,
		...
		for(ii2=0; ii2<element.array[ii1].node_num; ii2++){
			RC_TRY( realloc_bc_array(shape_rest) );
	...
	RC_TRY( clean_bc_array(shape_rest) );
	RC_TRY( compress_bc_array(shape_rest) );
...
RC
shape_trac_eigen_value (ELEMENT_ARRAY surf, DISP_ARRAY mode,
                        NODE_ARRAY node, ELEMENT_ARRAY element,
                        MATERIAL_PROP_ARRAY material,
                        PHYSICAL_PROP_ARRAY physical,
                        double lambda, BC_ARRAY *sens_trac)
{
	int ii1, ii2, ii3;
	WORK_SET ws;
	...
	for(ii1=0; ii1<surf.size; ii1++){
		...
		elem_index = search_element_label(element, surf.array[ii1].i_info[0]);
		if(elem_index < 0) return(SEEK_ERROR_RC);
		mat_index = search_material_prop_element(material,
		...
		for(ii2=0; ii2<num_points; ii2++){
			...
			/* 感度 G の計算 */
			RC_TRY( local_points_stress_strain(element.array[elem_index],
		...
		for(ii2=0; ii2<surf.array[ii1].node_num; ii2++){
			int index = search_bc_node(*sens_trac, surf.array[ii1].node[ii2]);
...
RC
shape_trac_max_mises (ELEMENT_ARRAY surf, DISP_ARRAY disp,
                      DISP_ARRAY adj_disp,
                      NODE_ARRAY node, ELEMENT_ARRAY element,
                      MATERIAL_PROP_ARRAY material,
                      PHYSICAL_PROP_ARRAY physical,
                      double rho, BC_ARRAY *sens_trac, int reduce_flag)
{
	...
	for(ii1=0; ii1<surf.size; ii1++){
		...
		elem_index = search_element_label(element, surf_elem.i_info[0]);
		if(elem_index < 0) return(SEEK_ERROR_RC);
		mat_index = search_material_prop_element(material,
		                           physical, &(element.array[elem_index]));
		if(mat_index < 0) return(SEEK_ERROR_RC);
...
/*fprintf(stdout, "%3d:%3d:%15.7e %15.7e %15.7e\n"
		...
		for(ii2=0; ii2<surf_elem.node_num; ii2++){
			int index = search_bc_node(*sens_trac, surf_elem.node[ii2]);
			if(index < 0) return(SEEK_ERROR_RC);
			sens_trac->array[index].v_type.x = BC_FORCE;
			sens_trac->array[index].v_type.y = BC_FORCE;
			sens_trac->array[index].v_type.z = BC_FORCE;
...
/* コンプライアンスの形状偏微分から勾配 sens_trac を計算する．
 */
RC
shape_trac_compliance (ELEMENT_ARRAY surf, DISP_ARRAY disp,
                       NODE_ARRAY node, ELEMENT_ARRAY element,
                       MATERIAL_PROP_ARRAY material,
                       PHYSICAL_PROP_ARRAY physical,
...
/* 体積の形状偏微分から勾配 sens_trac を計算する．
 */
RC
shape_trac_volume (ELEMENT_ARRAY surf, NODE_ARRAY node,
                   BC_ARRAY *sens_trac, int reduce_flag)
{
	int ii1, ii2, ii3;
	WORK_SET ws;
...
RC
shape_sens_max_mises (ELEMENT_ARRAY surf, DISP_ARRAY disp,
                      DISP_ARRAY adj_disp,
                      NODE_ARRAY node, ELEMENT_ARRAY element,
                      MATERIAL_PROP_ARRAY material,
                      PHYSICAL_PROP_ARRAY physical,
                      double rho, SENSITIVITY_ARRAY *sens)
{
	int ii1, ii2, ii3;
	...
	RC_TRY( allocate_sensitivity_array(0, sens) );
	sens->size = 0;
	...
	for(ii1=0; ii1<surf.size; ii1++){
		...
		elem_index = search_element_label(element, surf.array[ii1].i_info[0]);
		if(elem_index < 0) return(SEEK_ERROR_RC);
		mat_index = search_material_prop_element(material, physical,
		...
		RC_TRY( set_local_node_points(element.array[elem_index].type,
		                              local_points) );
		for(ii2=0; ii2<surf.array[ii1].node_num; ii2++){
			RC_TRY( realloc_sensitivity_array(sens) );
	...
	RC_TRY( clean_sensitivity_array(sens) );
...
RC
max_mises_adj_force (NODE_ARRAY node, ELEMENT_ARRAY element,
                     MATERIAL_PROP_ARRAY material,
                     ...
                     double rho, BC_ARRAY *adj_force)
{
	...
	double force_v[3*MAX_NODE];
	double force_v_sum[3*MAX_NODE];
	double m_integ, volume;
	int mat_index;
	...
	RC_TRY( allocate_bc_array(node.size, adj_force) );
	for(ii1=0; ii1<node.size; ii1++){
	...
	RC_TRY( clean_bc_array(adj_force) );
	...
	for(ii1=0; ii1<element.size; ii1++){
		...
		for(ii2=0; ii2<num_point; ii2++){
			...
			mises_str = rho * mises_stress(local_stress);
			if(mises_str > exp_limit) mises_str = exp_limit;
			adj_strain = mul_scalar_vect3dr(exp(mises_str)/m_integ,
		...
		for(ii2=0; ii2<element.array[ii1].node_num; ii2++){
			int index = search_bc_node(*adj_force,
			                           element.array[ii1].node[ii2]);
			if(index < 0) return(SEEK_ERROR_RC);
	...
	RC_TRY( free2D(6, 3*MAX_NODE, &B_matrix) );
	RC_TRY( free2D(3*MAX_NODE, 6, &trans_B) );
	RC_TRY( free2D(3*MAX_NODE, 6, &BtD) );
...
RC
max_mises_stress (NODE_ARRAY node, ELEMENT_ARRAY element,
                  MATERIAL_PROP_ARRAY material, PHYSICAL_PROP_ARRAY physical,
...
/* 熱応力を考慮したミーゼス応力の最大値 */
RC
max_mises_stress_therm (NODE_ARRAY node, ELEMENT_ARRAY element,
                        MATERIAL_PROP_ARRAY material,
                        PHYSICAL_PROP_ARRAY physical,
...
RC
mises_integration (NODE_ARRAY node, ELEMENT_ARRAY element,
                   MATERIAL_PROP_ARRAY material, PHYSICAL_PROP_ARRAY physical,
                   DISP_ARRAY disp, double rho, double *integ_val, double *vol)
{
	int ii1, ii2;
	double **node_location = NULL;
	double **gauss_point = NULL;
...
/* 節点が disp だけ変動した場合の汎関数の変動量を感度に */
/* 対応する荷重 sens_force より予測                     */
RC
functional_variation (BC_ARRAY sens_force, DISP_ARRAY disp, double *delta_f)
{
	/* 実際はコンプライアンスの計算と同様 */
...
RC
volume_functional (NODE_ARRAY node, ELEMENT_ARRAY element, double *volume)
{
...
/* 外力による仕事 - 強制変位による仕事 */
RC
potential_functional (BC_ARRAY force, DISP_ARRAY disp,
                      BC_ARRAY rest, REACT_ARRAY react, double *potential)
{
	double comp1, comp2;
...
/* 強制変位による仕事 */
static RC
restdisp_work (BC_ARRAY rest, REACT_ARRAY react, double *compliance)
{
	int ii1;
	int index;
	long double sum = 0.0;
	...
	for(ii1=0; ii1<rest.size; ii1++){
		...
		index = search_react_node(react, rest.array[ii1].node);
		if(index < 0) return(SEEK_ERROR_RC);
...
/* 外力仕事 */
RC
compliance_functional (BC_ARRAY force, DISP_ARRAY disp, double *compliance)
{
	int ii1;
	int index;
	long double sum = 0.0;
	...
	for(ii1=0; ii1<force.size; ii1++){
		...
		index = search_disp_node(disp, force.array[ii1].node);
		if(index < 0) return(SEEK_ERROR_RC);
...
/*               それ以外は粘性係数を代入                   */
RC
dissipation_functional (double fluid_param, NODE_ARRAY node,
                        ...
                        STRAIN_ARRAY strain_velo, double *dissipation)
{
	...
	*dissipation = 0.0;
	...
	for(ii1=0; ii1<element.size; ii1++){
		...
		for(ii2=0; ii2<g_point_num; ii2++){
			...
			for(ii3=0; ii3<element.array[ii1].node_num; ii3++){
				index = search_strain_element_node(strain_velo,
	...
	RC_TRY( mm_free((void *)N) );
	RC_TRY( mm_free((void *)weight) );
...
RC
dens_sens_comp_disp (const double power, DISP_ARRAY disp,
                     NODE_ARRAY node, ELEMENT_ARRAY element,
                     MATERIAL_PROP_ARRAY material,
                     PHYSICAL_PROP_ARRAY physical,
                     SENSITIVITY_ARRAY *sens, MATERIAL_PROP_ARRAY dens)
{
	int ii1, ii2;
	int dim;
	...
	RC_NEG_CHK( dim = analysis_dim(element) );
	for(ii1=0; ii1<element.size; ii1++){
		double *weights = ws.vec_G[0];
		...
		for(ii2=0; ii2<num_point; ii2++){
			...
			det_J = determinant(dim, jacobi);
			if( det_J < 0.0)
				RC_TRY( warning_printf(30001, element.array[ii1].label, ii1) );
			RC_TRY( local_points_stress_strain(element.array[ii1],
...
/* 熱歪による仕事の感度（簡易テスト版） */
RC
shape_sens_therm (ELEMENT_ARRAY surf, DISP_ARRAY disp,
                  NODE_ARRAY node, ELEMENT_ARRAY element,
                  MATERIAL_PROP_ARRAY material, PHYSICAL_PROP_ARRAY physical,
                  BC_ARRAY temp, DEFAULT_BC_ARRAY def_temp,
                  SENSITIVITY_ARRAY *sens)
{
	...
	static double **local_points;
	STRESS_STRAIN local_stress, local_strain;
	STRESS_STRAIN local_stress2, local_strain2;
	int elem_index;
	...
	for(ii1=0; ii1<surf.size; ii1++){
		...
		for(ii2=0; ii2<surf.array[ii1].node_num; ii2++){
			...
			RC_TRY( local_points_stress_strain(element.array[elem_index],
			             local_points[ii3], node, disp, material, physical,
			             &local_stress, &local_strain) );
			RC_TRY( local_points_stress_strain_therm(element.array[elem_index],
			             local_points[ii3], node, disp, material, physical,
...
RC
shape_sens_compliance_disp (ELEMENT_ARRAY surf, DISP_ARRAY disp,
                            NODE_ARRAY node, ELEMENT_ARRAY element,
                            MATERIAL_PROP_ARRAY material,
                            PHYSICAL_PROP_ARRAY physical,
                            SENSITIVITY_ARRAY *sens)
...
/* 応力、歪 -> 平均コンプライアンスの感度 */
RC
shape_sens_compliance_ss (ELEMENT_ARRAY surf, STRESS_ARRAY stress,
                          STRAIN_ARRAY strain, SENSITIVITY_ARRAY *sens)
{
	int ii1, ii2;
	int index_stress;
	int index_strain;
	...
	for(ii1=0; ii1<surf.size; ii1++){
		...
		for(ii2=0; ii2<surf.array[ii1].node_num; ii2++){
			...
			index_stress = search_stress_element_node(stress,
			                           surf.array[ii1].i_info[0],
			                           surf.array[ii1].node[ii2]);
			index_strain = search_strain_element_node(strain,
			                           surf.array[ii1].i_info[0],
			                           surf.array[ii1].node[ii2]);
			...
			sens->array[sens->size].s = -(
			   stress.array[index_stress].v.x  *strain.array[index_strain].v.x
			 + stress.array[index_stress].v.y  *strain.array[index_strain].v.y
			 + stress.array[index_stress].v.z  *strain.array[index_strain].v.z
			 + stress.array[index_stress].v.yz *strain.array[index_strain].v.yz
			 + stress.array[index_stress].v.zx *strain.array[index_strain].v.zx
			 + stress.array[index_stress].v.xy *strain.array[index_strain].v.xy
			);
			(sens->size)++;
		}
	}
...
/* strain : ひずみ流速 */
RC
shape_sens_dissipation (double fluid_param, ELEMENT_ARRAY surf,
                        ...
                        SENSITIVITY_ARRAY *sens)
{
	...
	for(ii1=0; ii1<surf.size; ii1++){
		...
		for(ii2=0; ii2<surf.array[ii1].node_num; ii2++){
			...
			sens->array[sens->size].element = surf.array[ii1].label;
			sens->array[sens->size].node = surf.array[ii1].node[ii2];
			index_v = search_strain_element_node(strain,
			                                     surf.array[ii1].i_info[0],
			                                     surf.array[ii1].node[ii2]);
			if(index_v < 0) return(SEEK_ERROR_RC);
			index_w = search_strain_element_node(adjstrain,
			                                     surf.array[ii1].i_info[0],
			                                     surf.array[ii1].node[ii2]);
			if(index_w < 0) return(SEEK_ERROR_RC);
			index_p = search_velo_node(flow, surf.array[ii1].node[ii2]);
			if(index_p < 0) return(SEEK_ERROR_RC);
			index_q = search_velo_node(adjflow, surf.array[ii1].node[ii2]);
			if(index_q < 0) return(SEEK_ERROR_RC);
...
RC
shape_sens_volume (ELEMENT_ARRAY surf, SENSITIVITY_ARRAY *sens)
{
	int ii1, ii2;
...
/*
 * Warning !!
 * sens2traction() is deprecated and should not be used in newly-written code.
 * See [azlib-devel:00025].
 *
 ...
 */
RC
sens2traction (NODE_ARRAY node, ELEMENT_ARRAY surf, BC_ARRAY shape_rest,
               SENSITIVITY_ARRAY sens, BC_ARRAY *trac, int reduce_flag)
{
	int ii1, ii2;
	double p[MAX_NODE];
	int p_size;
	...
	for(ii1=0; ii1<(surf.size); ii1++){
		...
		for(ii2=0; ii2<p_size; ii2++){
			index = search_sensitivity_element_node(sens,
		...
		for(ii2=0; ii2<tmp_surf.node_num; ii2++){
			index = search_bc_node(*trac, tmp_surf.node[ii2]);
			if(index < 0) return(SEEK_ERROR_RC);
	...
	for(ii1=0; ii1<(shape_rest.size); ii1++){
		...
		index = search_bc_node(*trac, shape_rest.array[ii1].node);
		if(index < 0) return(SEEK_ERROR_RC);
...
RC
add_disp_node (NODE_ARRAY node, int disp_num,
               const double *factors, const DISP_ARRAY *disps)
{
	int ii1, ii2;
	int index;
	VECT3D *sum;

	sum = (VECT3D *)mm_alloc((node.size)*sizeof(VECT3D));
	...
	for(ii1=0; ii1<disp_num; ii1++){
		for(ii2=0; ii2<disps[ii1].size; ii2++){
			...
			index = search_node_label(node, disps[ii1].array[ii2].node);
			if(index < 0) return(SEEK_ERROR_RC);
...
RC
max_pr_strain_disp (int str_num, const double factors[],
                    NODE_ARRAY node, ELEMENT_ARRAY element,
                    const DISP_ARRAY disp[], double *max_str, int sol_ss_flag)
{
	int ii1, ii2, ii3;
	double **local_points;
	STRESS_STRAIN local_strain;
	VECT3DR ss_v;
	double pr_val[3];
	double abs_v;
	...
	for(ii1=0; ii1<element.size; ii1++){
		...
		if(sol_ss_flag & SOL_SS_CENTER){
			...
			for(ii2=0; ii2<str_num; ii2++){
				RC_TRY( local_points_strain(element.array[ii1],
				             local_points[0], node, disp[ii2], &local_strain) );
				ss_v.x += factors[ii2] * local_strain.v.x;
				ss_v.y += factors[ii2] * local_strain.v.y;
				ss_v.z += factors[ii2] * local_strain.v.z;
				ss_v.yz += factors[ii2] * (0.5*local_strain.v.yz);
				ss_v.zx += factors[ii2] * (0.5*local_strain.v.zx);
				ss_v.xy += factors[ii2] * (0.5*local_strain.v.xy);
			}
		...
		if(sol_ss_flag & SOL_SS_NODE){
			...
			for(ii2=0; ii2<element.array[ii1].node_num; ii2++){
				...
				for(ii3=0; ii3<str_num; ii3++){
					...
					ss_v.yz += factors[ii3] * (0.5*local_strain.v.yz);
					ss_v.zx += factors[ii3] * (0.5*local_strain.v.zx);
					ss_v.xy += factors[ii3] * (0.5*local_strain.v.xy);
				}
				RC_TRY( principal3d(ss_v, pr_val, NULL, NULL, NULL) );
				for(ii3=0; ii3<3; ii3++){
...
/* 最大主歪みの絶対値 strain[x].array[*].element と                */
/* strain[x].array[*].node の値は x によらず一定でなければならない */
RC
max_pr_strain (int str_num, const double *factors,
	...
	*max_str = 0.0;
	for(ii1=0; ii1<(strain[0].size); ii1++){
		...
		for(ii2=0; ii2<str_num; ii2++){
			...
			ss.v.yz += factors[ii2] * (0.5*strain[ii2].array[ii1].v.yz);
			ss.v.zx += factors[ii2] * (0.5*strain[ii2].array[ii1].v.zx);
			ss.v.xy += factors[ii2] * (0.5*strain[ii2].array[ii1].v.xy);
		}
...
RC
make_trac_material_const (MATERIAL_PROP_ARRAY material,
                          MATERIAL_PROP_ARRAY *material_trac)
{
	int ii1;

	RC_TRY( allocate_material_prop_array(material.size, material_trac) );
	RC_TRY( copy_material_prop_array(material, material_trac) );
...
RC
make_trac_material (MATERIAL_PROP_ARRAY material,
                    MATERIAL_PROP_ARRAY *material_trac)
{
	int ii1;
