...
/* $Id: extract_surface.c 1057 2016-04-20 09:39:12Z hayashi $ */
...
/* 2次元用 */
typedef struct {
	ELEM_TYPE type; /* ELEM_TRI, QUAD, TETRA, PENTA, HEXA (1, 2) */
	int node_num;
...
/* elem が3次元要素なら表面を，2次元要素なら境界線を surf に格納する．
 */
RC
...
/* keys を用いて hash のインデックスを計算するハッシュ関数．
 */
static int
                 ...
                 int key_size, const int *keys)
{
	...
	/* 最初にハッシュテーブルにデータが入力されるときは，節点の情報を
	 * 格納する． */
...
/* keys1 が keys2 と等しければ，1 を返す． 
 * そうでなければ 0 を返す．
 * それぞれの keys はソートされていることを前提とする．
 */
static int
...
#if 0 /* ハッシュテーブルを表示してみる．*/
	int ii1, ii2;
	FILE *fp = get_log_file(1);
	for(ii1=0; ii1<hash_size; ii1++){
		fprintf(fp, "\nhash[%d]\n", ii1);
		fprintf(fp, "  elem_type: ");
		print_elem_type(fp, hash[ii1].type);
		fprintf(fp, "  node_num : %d\n", hash[ii1].node_num);
		for(ii2=0; ii2<MAX_NODE_LIGHT; ii2++){
			fprintf(fp, "  node[%d] : %d\n", ii2, hash[ii1].node[ii2]);
		}
		fprintf(fp, "  from_num : %d\n", hash[ii1].from_num);
		for(ii2=0; ii2<2; ii2++){
			fprintf(fp, "  from[%d] : %d\n", ii2, hash[ii1].from[ii2]);
		}
		for(ii2=0; ii2<2; ii2++){
			fprintf(fp, "  from_f[%d] : %d\n", ii2, hash[ii1].from_f[ii2]);
		}
		fprintf(fp, "  tmp : %d\n", hash[ii1].tmp);
	}
	return(END_RC);
               ...
               const int *keys, int label, ELEM_TYPE type, int number)
{
	...
	/* keysに応じたhashのインデックスを計算する． */
	RC_NEG_CHK( index = elem_hash_index(hash_size, hash, key_size, keys) );

	hash[index].from_num ++;
	/* 同じ面（辺）を共有する要素が3つ以上になることはないはずなので，
	 * from_num が 1 か 2 以外になったときはオーバーフローエラーとする． */
...
/* set_hash_*() 系関数
 *
 * 要素 elem の面（辺）ごとにkeysを作成し，set_hash_face() でhashを更新する．
 * numberの正体が不明．
 */
