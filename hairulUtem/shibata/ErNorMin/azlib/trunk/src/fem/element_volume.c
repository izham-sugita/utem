...
/* $Id: element_volume.c 842 2006-12-04 06:21:20Z sasaoka $ */
...
static RC Gauss_const4volume(ELEM_TYPE type, double **points,
                             double *weight, int *num_point);

RC
total_element_volume (ELEMENT_ARRAY element,
...
RC
element_volume (ELEMENT *element, NODE_ARRAY node)
{
	...
	for(ii1=0;ii1<num_point;ii1++){
		...
		if(f < 0.0) RC_TRY( warning_printf(3001, element->label, ii1) );
...
static RC
Gauss_const4volume (ELEM_TYPE type, double **points,
                    double *weight, int *num_point)
{
	switch(type){
	case ELEM_RBAR:
