...
/* $Id: dxf_utl.h 842 2006-12-04 06:21:20Z sasaoka $ */
...
RC output_dxf_3dface_lay(FILE *fp, char **layer, int num_layer,
                         ELEMENT_ARRAY elem, NODE_ARRAY node);
RC output_dxf_polyline(FILE *fp, ELEMENT_ARRAY element,
                       NODE_ARRAY node);
RC output_dxf_polyline_lay(FILE *fp, char *layer,
                            ELEMENT_ARRAY elem, NODE_ARRAY node);

RC input_dxf(FILE *fp, ELEMENT_ARRAY *element, NODE_ARRAY *node);
RC input_dxf_3dface(FILE *fp, ELEMENT_ARRAY *element, NODE_ARRAY *node);
RC input_dxf_3dface_lay(FILE *fp, char ***layer, int *num_layer,
                        ELEMENT_ARRAY *element, NODE_ARRAY *node);

RC input_dxf_polyline(FILE *fp, ELEMENT_ARRAY *element, NODE_ARRAY *node);
