...
/* $Id: nonzero_cg.c 1059 2016-04-25 09:52:20Z hayashi $ */
                       ...
                       int mode_size, const double mode[])
{
	...
	if((matrix->row)[old_size].elem == NULL) return(ALLOC_ERROR_RC);
                ...
                int precondition, int verb_sw)
{
	...
	for(ii1=0; ii1<it_max; ii1++){
		...
		if((rel_error_b <= REL_ERROR1)||(conv_count >= 10)){
			if(verb_sw)
				RC_TRY( log_printf(5, "%6d: converged\n", ii1) );
			break;
		}
...
print_nonzero_matrix1a (FILE *fp, NONZERO_MATRIX matrix)
{
	...
	for(ii1=0; ii1<matrix.size; ii1++){
		...
		fprintf(fp, "row %d\n", ii1);
		for(ii2=0; ii2<matrix.row[ii1].size; ii2++){
			fprintf(fp, "  col %d:   value %e\n",
			             matrix.row[ii1].col[ii2], matrix.row[ii1].v[ii2]);
		}
		fprintf(fp, "\n");
	}
}

/* CRS行列 matrix にスカラー定数 fact をかける．
 */
void mul_scalar_nonzero_matrix1a (const double fact, NONZERO_MATRIX *matrix)
{
	...
	for(ii1=0; ii1<(matrix->size); ii1++){
		mul_scalar_nonzero_row(fact, &(matrix->row[ii1]));
	}
}

/* CRS行 row にスカラー定数 fact をかける．
 */
...
{
	RC_NEG_ZERO_CHK( A.size );
	if(A.size != B.size) return(ARG_ERROR_RC);
	if(A.size != C->size) return(ARG_ERROR_RC);

	int ii1, ii2;
	double *ptr;

	RC_TRY( copy_nonzero_matrix1a(A, C) );
	mul_scalar_nonzero_matrix1a(w1, C);

	for(ii1=0; ii1<B.size; ii1++){
		for(ii2=0; ii2<B.row[ii1].size; ii2++){
			RC_NULL_CHK( ptr = nonzero_matrix1a_ptr(C, ii1,
						B.row[ii1].col[ii2]) );
			*ptr += w2*B.row[ii1].v[ii2];
		}
	}
...
/* CRS行列 matrix を動的確保する．
 * matrix.size = total_dof, matrix.row[].size = 0 が代入される．
 * 各行は nonzero_matrix1a_ptr() で値を代入することを想定しており，
 * この関数内で確保は行わない．
 */
RC
...
/* row[col] がすでに確保してあるなら，その値のポインタを，
 * 確保してないなら，新たに確保してそのポインタを返す．       
 * 新たに確保した場合，返すポインタの col や diagonal_index なども格納する．
 *
 * この関数使用時は，allocate_nonzero_matrix() を事前に通す．   
 * 不備が生じた場合は，NULL が戻り値となる．
 */
double *
nonzero_row_ptr (NONZERO_ROW *n_row, int row, int col)
{
	int ii1;

	if(0 > n_row->size) return(NULL);
	...
	n_row->v = (double *)mm_realloc(n_row->v, (n_row->size)*sizeof(double) );
...
/* a[row][col] がすでに確保してあるなら，その値のポインタを，
 * 確保してないなら，新たに確保してそのポインタを返す．       
 * 新たに確保した場合，返すポインタの col や diagonal_index なども格納する．
 *
 * この関数使用時は，allocate_nonzero_matrix() を事前に通す．   
...
{
	return( nonzero_row_ptr(&(matrix->row[row]), row, col) );
}

/* 動的確保したCRS行列 matrix を解放する．
 */
RC
...
/* 上に offset_row 行，左に offset_col 行空けて，source を copy にコピーする．
 * オフセットにあった値は残る．
 * それ以外の値は上書きする．
 *
 * copy は確保してから渡す．
...
{
	/* 行が copy に収まり切らない場合はエラーとする． */
	if(offset_row + source.size > copy->size){
		return(ARG_ERROR_RC);
	}
	...
	for(ii1=0; ii1<source.size; ii1++){
		for(ii2=0; ii2<source.row[ii1].size; ii2++){
			ptr = nonzero_matrix1a_ptr(copy, offset_row+ii1,
						offset_col+source.row[ii1].col[ii2]);
			RC_NULL_CHK( ptr );
			*ptr = source.row[ii1].v[ii2];
		}
	}
...
/* CRS行列 source を転置して dest に代入する．
 * dest は確保してから渡す．
 */
RC transpose_nonzero_matrix1a (NONZERO_MATRIX source, NONZERO_MATRIX *dest)
{
	RC_NULL_CHK( dest );
	...
	for(ii1=0; ii1<source.size; ii1++){
		for(ii2=0; ii2<source.row[ii1].size; ii2++){
			ptr = nonzero_matrix1a_ptr(dest, source.row[ii1].col[ii2], ii1);
			RC_NULL_CHK( ptr );
			*ptr = source.row[ii1].v[ii2];
		}
	}
...
RC delete_zero_matrix1a (NONZERO_MATRIX *matrix)
{
	...
	for(ii1=0; ii1<(matrix->size); ii1++){
		for(ii2=0; ii2<(matrix->row[ii1].size); ii2++){
			/* 0 でなければスキップする． */
			if(fabs(matrix->row[ii1].v[ii2]) >= DBL_MIN) continue;
...
nonzero1a_scaling (NONZERO_MATRIX *matrix, double *vector)
{
	...
	for(ii1=0; ii1<matrix->size; ii1++){
		...
		if(matrix->row[ii1].col == NULL) return(ALLOC_ERROR_RC);
		matrix->row[ii1].v = (double *)mm_realloc(matrix->row[ii1].v,
	                                matrix->row[ii1].size * sizeof(double) );
		if(matrix->row[ii1].v == NULL) return(ALLOC_ERROR_RC);
...
RC convert_nonzero1a2double (int row, int col, NONZERO_MATRIX source, 
		double **dest)
{
	RC_NEG_ZERO_CHK( source.size );
	RC_NULL_CHK( dest );

	int ii1, ii2;

	init_matrix(row, col, dest);
	for(ii1=0; ii1<source.size; ii1++){
		for(ii2=0; ii2<source.row[ii1].size; ii2++){
			dest[ii1][source.row[ii1].col[ii2]] = source.row[ii1].v[ii2];
		}
	}
		...
		NONZERO_MATRIX *dest)
{
	...
	int ii1, ii2;
	RC_TRY( allocate_nonzero_matrix1a(row, dest) );
	for(ii1=0; ii1<row; ii1++){
		RC_TRY( allocate1D(col, &(dest->row[ii1].v)) );
		RC_TRY( allocate1I(col, &(dest->row[ii1].col)) );
		for(ii2=0; ii2<col; ii2++){
			dest->row[ii1].col[dest->row[ii1].size] = ii2;
			dest->row[ii1].v[dest->row[ii1].size] = source[ii1][ii2];
			if(ii1 == ii2){
				dest->row[ii1].diagonal_index = dest->row[ii1].size;
			}
			dest->row[ii1].size++;
		}
	}
