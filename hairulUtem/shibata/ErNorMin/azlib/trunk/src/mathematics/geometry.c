...
/* $Id: geometry.c 1082 2016-11-17 14:12:12Z hayashi $ */
...
/* p[4] で表される4面体の内部に point があるか確認する．
 *
 * 4面体の1面を底面とし，point を頂点とする4面体を4つ計算する．
 * その和がもとの四面体の体積に等しければ，point が四面体の内部にあると判定
 * する．
 */
int
is_inside_tetra (const VECT3D p[], VECT3D point)
{
    double volume;
    double volume1, volume2, volume3, volume4;
    double sum;

    volume = tetra_volume(p[0], p[1], p[2], p[3]);
    volume1 = tetra_volume(point, p[0], p[1], p[2]);
    volume2 = tetra_volume(point, p[0], p[1], p[3]);
    volume3 = tetra_volume(point, p[0], p[2], p[3]);
    volume4 = tetra_volume(point, p[1], p[2], p[3]);
    sum = volume1 + volume2 + volume3 + volume4;

    if(nearly_eq(volume, sum)) return(1);
...
/* p1, p2, p3, p4 を頂点とする4面体の体積を計算する．
 *
 * p1を始点とするベクトルを3つ計算し，それらのスカラー3重積の6分の1を計算する．
 */
double
tetra_volume (VECT3D p1, VECT3D p2, VECT3D p3, VECT3D p4)
{
    VECT3D v1 = sub_vect3d(p2, p1);
    VECT3D v2 = sub_vect3d(p3, p1);
    VECT3D v3 = sub_vect3d(p4, p1);

    return(fabs(scalar_triple_product3d(v1, v2, v3)/6.0));
}
