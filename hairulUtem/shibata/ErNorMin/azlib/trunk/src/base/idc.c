...
/* $Id: idc.c 1066 2016-05-19 09:53:33Z hayashi $ */
...
/* 入力源 fp から1行読み込み，データをIDC共用体の配列 data に格納する．
 * 読み込むデータ型を coord で指定する．
 *
 * たとえば，
 *   1 2 1.000
 * という行に対して
 ...
 * が代入される．
 *
 * forループ内で用いて，ファイルを1行ずつ読み込んで変数に値を格納すると
 * いった使い方をする．
 */
RC
fgetidc (FILE *fp, const char coord[], IDC data[])
{
...
/* 文字列 buf を読み込み，データをIDC共用体の配列 data に格納する．
 * 読み込むデータ型を coord で指定する．
 *
 * たとえば，
 *   buf = "1 2 1.000"
 ...
 */
RC
sgetidc (const char buf[], const char coord[], IDC data[])
{
...
static int
idc_get_c (const char buf[], char tmps[], int ptr)
{
	int ii = 0;
 ...
 */
static int
idc_get_a (const char buf[], char tmps[], int ptr)
{
	int ii = 0;
...
static int
idc_get_d (const char buf[], char tmps[], int ptr)
{
	int ii = 0;
...
static int
idc_get_i (const char buf[], char tmps[], int ptr)
{
	int ii = 0;
	int vflag = 0;  /* 数値らしきものが現れたら1 */
	int sflag = 0;  /* 符号が現れたら1 */
